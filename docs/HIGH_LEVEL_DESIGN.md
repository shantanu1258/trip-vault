...
**Implementation status:** Timeline-first application and the accepted event-form/document redesign are complete in the local working tree. Existing Supabase projects apply every pending immutable migration in filename order. Fresh projects use the canonical two-phase `supabase/TRIP_VAULT_COMPLETE_SETUP.sql` workflow documented in `supabase/README.md`. Remote, phone, desktop, and airplane-mode acceptance remain pending.
...
| Automated verification | Verified 2026-09-17 baseline: 83 Vitest files and 480 tests pass, and the production build passes; the previously verified type-check and formatting check remain green; remote SQL, Storage, and device acceptance remain separate release gates |
| Existing Supabase project | Do not use the complete setup as an upgrade script. Apply every not-yet-run immutable migration in filename order through `202609170001_journey_timeline_and_timezone.sql`, then run the schema smoke test |
...
1. Apply every pending existing-project migration in filename order through `202609170001_journey_timeline_and_timezone.sql`, or complete both phases of the fresh-project setup described in 9.3, then rerun the schema smoke test.
