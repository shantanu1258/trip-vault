...
**Implementation status:** Trips-first shell, unified expandable timeline and filters, explicit nested return navigation, document-first event details, typed booking heroes, animated event identity, and in-app alerts are implemented. Existing Supabase projects apply pending immutable migrations in filename order; fresh projects follow the two-phase setup in `supabase/README.md`. These September 19 client refinements require no new schema or dependency beyond the earlier bundled font addition. Web Push remains proposed, not deployed. Local responsive checks do not replace remote, multi-device, or airplane-mode acceptance.
...
Within an event, existing documents precede secondary add actions and are grouped by usage rather than manual order: Everyone first/open, traveler groups next with contextual expansion, then unassigned. Empty states promote upload/attach. Editors retain authorized attachment and unlinking without per-document move controls.
...
| HLD-072 | Optional Web Push            | Explore per-device Web Push subscriptions, server-side reminders, and expiry/deduplication without replacing offline-capable in-app alerts | Proposed |

Web Push has no deployed subscription table, sending Edge Function, or scheduler in this repository. Proposed delivery would use Supabase Cron and a protected Edge Function plus browser push services, with secrets held server-side, explicit device consent, per-user authorization, and notification expiry. Offline devices cannot receive new push; deferred delivery is best-effort within its expiry. Free-project pausing and browser/OS restrictions prevent alarm-like delivery guarantees. Implementation and deployment require a separate change.
