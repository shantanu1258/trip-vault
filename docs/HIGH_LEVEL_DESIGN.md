...
**Implementation status:** Timeline-first application, progressive event forms, compact agenda navigation, ordered Cab stops, grouped event documents, readiness details, and restrained airline accents are implemented. Existing Supabase projects apply every pending immutable migration in filename order. Fresh projects use the canonical two-phase `supabase/TRIP_VAULT_COMPLETE_SETUP.sql` workflow documented in `supabase/README.md`. Remote, phone, desktop, and airplane-mode acceptance remain pending.
...
| Area                      | Current state                                                                                                                                                                                                                                        |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Application               | Timeline-first React PWA with progressive event forms, route-backed agenda navigation, in-app document preview, ordered Cab stops, and airline accents is implemented                                                                                |
...
| Existing Supabase project | Do not use the complete setup as an upgrade script. Apply every not-yet-run immutable migration in filename order through `202609170002_cab_journey_stops.sql`, then run the schema smoke test                                                       |
...
A timeline event exposes one labelled primary document shortcut—such as **Open ticket** or **Open voucher**—without replacing the whole-card event-details action. The shortcut chooses an explicitly linked event document first, then a booking document, and applies the active traveler filter before presentation. Booking details also let an editor start an upload from a particular Train, Bus, Ferry, or Cab leg; the upload carries that `journey_leg_id`, defaults to a journey-ticket purpose, and remains available only to the traveler context allowed by the document assignment.

Within an event, linked documents are grouped by traveler usage rather than manual card order. **Everyone** appears first, followed by selected-traveler groups in traveler creation order and then unassigned documents. Groups begin expanded and may be collapsed; editors may attach, upload, or unlink documents without per-document move controls.
...
The route-backed **Trip agenda** is the compact navigator for a long timeline. It groups entries by local event date, expands current and future dates by default, keeps past dates compact, filters by travel/stay/activity and date, and provides one shortcut to the active entry. Selecting an agenda row or search result closes the navigator and focuses the same timeline card. Trip search accepts event and booking metadata plus flexible local-date aliases such as a weekday, `28 Feb`, a month name, or an ISO date; a date result jumps to the first event under that date heading.

All timeline focus paths use the same positioning contract: the phase/date heading and target card settle immediately below the sticky header, rather than in the vertical center. After settling, the card receives a brief two-pulse border cue—the active accent for the current entry and a light-blue cue for another entry. Reduced-motion preference removes smooth scrolling and animated emphasis without removing the target position.
...
For an existing project, do not use the complete setup file as an upgrade script. Apply every missing immutable file under `supabase/migrations/` in filename order through the current tail, `202609170002_cab_journey_stops.sql`, then run the smoke test. A deployment may have applied any prefix of this history, so previously released migrations remain unchanged. `202609130005` still requires an active `app_admins` row and the regional release from `202609130002`; establish those prerequisites instead of bypassing its guard.
...
| HLD-063 | Compact timeline navigation             | Use one route-backed agenda with local-date groups, current shortcut, category/date filters, and direct timeline focus; make weekday and flexible date aliases searchable                                                                                                                          | Accepted             |
| HLD-064 | Timeline focus feedback                 | Settle every focused heading/card below the sticky header, pulse the card briefly after arrival, use the current accent only for the active item and light blue for other targets, and honor reduced motion                                                                                        | Accepted             |
...
| HLD-066 | Restrained airline accents              | Keep flight surfaces semantically neutral and theme-safe; apply an approved airline color only to narrow rails, dots, and hero markers, including one dot per connection                                                                                                                           | Accepted             |
...
| Schema/client mismatch                | A deployed client can reference tables, functions, triggers, policies, or catalog releases missing from an older Supabase project | For an existing project, run every pending immutable migration in filename order through the current tail; for a fresh project, complete both canonical setup phases; then execute the schema smoke test before client testing |
...
1. Apply every pending existing-project migration in filename order through `202609170002_cab_journey_stops.sql`, or complete both phases of the fresh-project setup described in 9.3, then rerun the schema smoke test.
...
| Event-form data migration            | `supabase/migrations/202609140001_event_form_data_model.sql`                                                                                                                                       | Reservation/participant state, nullable journey arrival, validated mode details, per-leg traveler allocations, and atomic hotel milestones |
| Journey placement/timezone migration | `supabase/migrations/202609170001_journey_timeline_and_timezone.sql`                                                                                                                               | Journey Before/After placement and Domestic journey/flight timezone edits                                                                  |
| Cab-stop migration                   | `supabase/migrations/202609170002_cab_journey_stops.sql`                                                                                                                                           | Ordered Cab stops, same-trip event links, and stop-specific costs                                                                          |
| Agenda, focus, and search            | `src/features/trips/TripAgendaSheet.tsx`, `src/features/timeline/model.ts`, `src/features/timeline/scroll.ts`                                                                                      | Compact date navigation, flexible date aliases, shared target positioning, and focus feedback                                              |
