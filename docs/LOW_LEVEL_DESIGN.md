...
Web Push is implemented locally, behind `VITE_PUSH_ENABLED=false`. Profile manages permission, registration/revocation, three device preferences, and tests. `push_subscriptions` has owner RLS and column-limited updates; registration uses a validated, ten-device-limited RPC. Browsers cannot access `push_jobs`. Event/cost writes and booking updates queue generic changes for other active members; claim-time reminders cover explicitly timed events starting within one hour. Send-time checks revalidate source, membership, preferences, and reminder timing/status. Dates, all-day/inferred times, and task/visa/manual reminders are excluded.
...
| ---------------------------------------- | ---------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TypeScript and production build          | 2026-09-22                                                       | Pass, including PWA generation                                                                                                                                              |
| `npm test -- --reporter=dot`             | 2026-09-22                                                       | 811 tests across 136 files pass                                                                                                                                             |
| Complete SQL installer                   | 2026-09-22, isolated PGlite with Supabase Auth/Storage stand-ins | Fresh installation, existing-schema refusal, deferred/admin-gated catalogue publication and safe publication retry pass                                                     |
| SQL smoke scripts 001–007                | 2026-09-22, isolated PGlite                                      | All pass, including document permissions, Planning/Moments, current features and admin draft discard/publication boundaries                                                 |
| HLD / LLD formatting                     | 2026-09-22                                                       | Prettier verification before commit                                                                                                                                         |
...
| Hosted SQL / Storage / device acceptance | Separate deployment gates                                        | Apply only missing SQL sections after backup; real-device sharing, installed-PWA airplane mode, multi-account permissions and post-push deployment still require acceptance |
