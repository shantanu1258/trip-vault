...
> **September 23 update:** Pulled quick/connected expense entry, the full Expenses route and stock-palette border improvements are documented below. Push change snapshots and specific notification text are implemented locally; hosted migration, sender/frontend deployment and real-device acceptance remain pending.
...
> **Current contract:** Reconciled with the September 22 document, demo, Admin and single-installer implementation. See the [changelog](../CHANGELOG.md), section 16.1 for local verification, and [PUSH_SETUP.md](PUSH_SETUP.md) for push deployment status. Historical acceptance notes below do not imply that the latest SQL or client build is deployed.
...
│   ├── tests/
│   └── migrations/                 # Incremental changes for existing deployments
├── scripts/
├── .env.example
├── package.json
├── tsconfig.json
...
| `/trips/:tripId/expenses`              | Trip member               | Full expenses with search, category/traveler filters, details and quick entry               | Cached costs with supported offline writes; child Moment/Cab-stop dependencies precede cost synchronization                                      |
...
An unpublished row with `status = 'retired'` and no `version_number` is a discarded draft. `discard_config_draft` checks administrator access and draft state under a release-table lock, retains rows/assets, and appends a `retired` audit event with `discarded_draft: true`. Active release listings exclude these rows. Publication shares this locking boundary and refuses discarded drafts. Change previews compare stable-key content, excluding row identity and update timestamps; previous-version selection uses publication versions rather than the copied-source ID.
...
This retained focus table does not make a trip current. Attention helpers validate membership and their contextual window; RootRoute uses the separate ten-day launch rule and does not select an overlapping trip from saved focus.
...
Standalone Add Cost and unlinked Edit Cost offer No link, Event, or Document. Event-linked expenses keep their source association; a new event-associated expense starts with that event's travelers, while edits retain their saved split. Document uploads reuse `UploadDocumentForm` with the receipt preset and unchanged visibility controls. Cancelling the cost leaves an uploaded document in Vault. Payment status and Paid by share a two-column row on all screen sizes. The complete setup enforces same-trip, document-access, and exclusive-association checks. Offline receipt-linked costs depend on the document association outbox operation; online saves reject unfinished document synchronization rather than creating a dangling link.
...
| Compare releases or discard an unpublished draft              |                      Yes |                            No |
...
`AuthenticatedRoute` encodes the immediate requested internal route in `/sign-in?next=…`; `authReturnPath` rejects external/protocol-relative URLs and auth-route loops. `InvitationContinuation` sits outside the pathname-keyed boundary, captures only `/join` invitation codes while signed out, and retains one normalized code with a non-renewing 14-day deadline in localStorage. After login it resumes the ordinary Join page from any personal route, including the normal home page. Admin routes neither resume nor consume it. Storage failures retain the immediate URL fallback, but cannot guarantee continuation after navigating away.

`JoinPage` clears the pending browser record when it presents the invitation. All recipients confirm Join trip before a protected redemption attempt; retries are disabled, failures leave the code editable, and success invalidates trips and opens the joined trip. The code is not copied to account metadata or IndexedDB. Switching browsers/devices requires reopening the invitation.

`SignInPage` treats a signup session as immediate success, as expected with the current confirmation-disabled configuration. Signup and normal password sign-in prioritize the pending invitation as the next route. No OTP, email-template change or custom callback is introduced. If the backend unexpectedly requires confirmation and returns no session, the existing confirmation message remains and the invitation waits for a later successful login. See [INVITATION_SIGNUP_SETUP.md](INVITATION_SIGNUP_SETUP.md) for acceptance checks.
...
The Vault list shows the trip selector as a content-width subtitle (map icon, trip name, chevron) under the Document Vault heading on the Trip documents tab, followed by search and one Filters disclosure. Document types stay in an always-visible, unboxed horizontal chip bar with a hidden scrollbar. Only individual-event and traveller selectors expand under Filters; collapsed applied filters show removable labels and a clear action that leaves the selected type intact. The active trip remains visible while the default toolbar avoids a grid of form controls. Trip navigation passes an explicit `trip` URL parameter; direct Vault entry defaults to the current `tripActivitySpan` only after successful schedule reads. `trip=all` is retained explicitly so automatic defaults cannot override that choice. All filters remain in the URL for browser Back, and changing trips clears event/traveller selections. Event filtering includes direct event/document references plus documents belonging to the event's booking, matching event details. Shared documents remain visible for a selected traveller. Personal documents remain independent. Add document carries the selected authorized trip into the existing upload flow; no new schema is required.

Current-trip calculation now uses `tripActivitySpan`: itinerary bounds are combined with dated readiness tasks, relative task schedules resolved against valid event anchors, and the user's trip-linked manual reminders. Date-only tasks begin at local midnight. Completed events/tasks/reminders still establish the original start; not-required tasks, cancelled/deleted event anchors, undated tasks and unrelated reminders are excluded. `listReminders(true)` reads completed reminder history into a separate cache/query key, so normal Alerts still loads incomplete reminders only. Offline history is merged with locally created reminders. Trip travel dates do not determine this Vault default; the latest-starting qualifying activity span wins overlaps. The picker itself still lists attachable itinerary events, not task/reminder records.

`AddDocumentPage` loads authorized trips and cache-backed itinerary, readiness and reminder queries. Archived trips are excluded. The latest qualifying activity span becomes the default unless the user already chose a destination or entered through Personal; creation time and ID break ties deterministically. Failed schedule reads do not guess. Trip changes reset event, type and traveller. Owners/editors can select an event; booked events reuse their booking and unambiguous single-leg association. Document type and traveller appear immediately below the event picker: flights suggest Boarding pass, stays suggest Hotel confirmation, journeys suggest Journey ticket and activities suggest Activity confirmation. Traveller defaults to Everyone, including for boarding passes; selecting a person passes a selected-traveller assignment into review. Type changes preserve the traveller choice. The existing upload form retains explicit save, editable assignment and separate visibility review. Event links use `attachDocumentsToEvent`, including offline upload dependencies; a failed link retries the already-saved document rather than uploading another copy.

`QuickFlightSeats` appears in editable flight event sheets, one compact disclosure per flight leg. Participants and focused traveler filter its rows. It lazily loads the existing flight-traveler record, saves only a seat edit while retaining the loaded boarding group/ticket number, and invalidates all flight-traveler query variants so timeline summaries and detailed pages agree. Existing API/outbox handling provides offline saves. The shared `FlightSeatsEditor` owns the layout, example placeholder, loading/error states and seat normalization; demo uses it with in-memory state instead of a live query/mutation. Quick and detailed seat inputs share theme-aware `.seat-input` placeholder styling.
...
| Readiness requirement     | Opens the read-first task sheet                                                              | Checkbox alone completes/undoes; edit/archive/status and links remain independent and role-gated |
...
| `TripPage` timeline composition                                      | Complete timeline, date/type filters, active-event scroll, event detail sheet, people/sharing sheet, and Details switch                                                                                                                             |
...
Broad decorative gradients are avoided. The implemented current-event card uses a faint semantic accent gradient/tint; artwork must not compete with text or urgent information.

### 11.4 Timeline and trip details

#### Full expenses and quick entry

`TripExpensesPage` serves `/trips/:tripId/expenses`; Home and Trip header totals navigate here. `search`, `category` and `traveler` parameters retain filter state; missing traveler selection falls back to the trip's saved traveler focus. Filtering matches title/category/payment state/payer name, selected category, and traveler payer/participant membership. Costs sort newest first with a title tie-break; filtered totals remain separated by currency. Existing `CostDetailsSheet` and `TripExpensesContent` provide read-first details, role-gated actions and opt-in balances. Trip details retains a compact expense preview.

`QuickAddCostForm` requires a nonblank title, positive numeric amount and allowed category; it uses `amountStringToMinor`, the trip currency, Paid status and all active travelers. `CostEventConnectionFields`/`useCostEventConnection` are shared with the full form, selecting an event and an optional Activity Moment or Cab stop. Changing events clears the child selection. Creating a child uses the event timezone or first Cab leg; the returned child ID is retained before cost persistence so a failed cost save can retry without recreating it. These steps are not one atomic server transaction. Cost association updates carry explicit nullable links, clear incompatible child links, and invalidate relevant collection queries. Existing databases add Snacks/Gift enum values through `202609220005_quick_cost_categories.sql`.

`TripTimeline` replaces the separate Agenda. It groups the shared timeline projection by local date, opens dates initially, expands only the current entry initially, and permits several expanded event cards. `TimelineFilters` presents Dates / Event types / Display as a phone sheet or larger-screen drawer. Date/type/display edits remain drafts until Apply; close, Escape, and Back discard drafts. Reset and remove-filter actions preserve the defined expansion behavior. `eventTypeChoices.ts` supplies the common Add Event/filter vocabulary, combining hotel check-in/out under Hotel. Presentation state is session-local and scoped to profile, trip, and traveler.

`searchTrip()` creates local-date results alongside event/booking/document/traveler results. Date aliases include ISO/slashed dates, long/short weekdays, US/GB day-month order, month names, and optional year. `eventTypeSearchTerms()` adds visible type labels, plurals, and aliases such as hotel/stay, ferry/boat, cab/taxi, and meal/restaurant to timeline and booking matches, with readiness/preparation terms for tasks. A date result targets its first chronological entry. The header search control reaches the Timeline search region; explicit jumps reveal the destination date/card and clear conflicting filters.

`scrollTimelineEventIntoView()` targets the card rather than the preceding date/phase heading. It reserves both sticky-header heights plus 20px (minimum 104px), excludes visible floating actions and bottom navigation with a 16px gap, and centers the trigger/heading in the remaining visual viewport. Tall cards center their heading, not their entire body. One smooth window scroll is followed by `scrollend`; the 650ms initial fallback is reset to 160ms after each scroll event, preventing highlights halfway through long jumps. The CSS pulse has three peaks over 3500ms, with a 2px inset stroke and 4px translucent outer ring, returning to the target's resting border between peaks. Brand is the default focus color, current-event `coral` and current-task `warning` preserve semantic emphasis. Reduced motion selects instant scrolling and effectively disables animation.

`/trips/:tripId` opens in Timeline view unless `?view=details` is present.

- The timeline renders the authorized chronological projection, subject to applied date/type and traveler filters. Dates and cards expand independently; collapsing a date preserves its cards' choices.
...
- Current events retain a faint accent tint, double-stroke border/outline, and trailing NEXT/NOW label. Current tasks retain amber TO DO treatment. Ordinary event cards use neutral borders; compact headers expand/collapse, and expanded bodies open the event sheet.
- Compact colored `EventTypeIcon` badges sit left of the text on all breakpoints; hotels use the bed icon. Disclosure chevrons remain top-right, with no header silhouette competing with them.
...
- Details uses Readiness, Next up, Reservations, Costs, People, Documents, Offline, Travel data, Notes and relevant management sections. The redundant Overview card is removed; Owner settings open through Edit trip. Next up has at most three events with explicit target intents. Reservations and Documents expose category counts and three-row previews linking to their full collections. The trip hero's currency summary links to itemized expenses.
...
#### Return and expansion contracts

`TripChildLink` and `tripChildScrollState` capture origin path/query/hash, view, nested parent context, and the selected card's scroll offset before opening details. `TripBackLink` uses an explicit replacement navigation to `tripReturnNavigation()`, never `navigate(-1)`; direct entries fall back to the trip. `returnScroll.ts` restores the original element offset or saved scroll. Modal → booking → Back restores the modal's origin stack, so closing the modal returns to the same timeline location. Collection and Trip details return positions are retained too.

Trip document rows and the “View all documents” link use this same capture path. `TripDocumentsPage` waits for its ranked list before restoring and keeps search/category/traveler filters in query parameters. Add event uses `useUnsavedEventGuard`: user field/picker/file/connection edits enable a discard confirmation; successful saves bypass it. A data-router `useBlocker` covers route/query Back transitions, and `beforeunload` requests the browser's native warning for document-level navigation. The discard confirmation does not create its own history entry. Cancelling retains in-memory form values; no persistent draft store or automatic draft save is implemented.

Fresh target/current/search intents clear stale `scrollRestore`. `TripPage` prioritizes an explicit target/current request over a saved entry position and reveals its date/card before measuring. Genuine restore intents retain their saved position. `revealExpandedTimelineEvent()` is separate: manual expansion moves only enough to clear headers/dock, leaves fitting cards alone, and aligns an oversized card below the shown headers.

#### Event identity and booking presentation

`EventSilhouette` renders decorative, pointer-transparent, aria-hidden artwork in existing surfaces. Expanded booking summaries use 80% of parent height; fallback expanded cards, modals, and heroes use 50%. Resting placement is bottom-right. Compact timeline headers use icons only, despite retained legacy header-transition support in the component. Flights enter from bottom-left along a 45-degree upward path; vehicles enter from the left; hotel suns rise; activity wheels rotate and meal lids open. Animations are one-shot on mount/expansion, consistent across breakpoints, and disabled under reduced motion.

`eventIconTone`, event-surface CSS, and `bookingEventType` share type identity. Summary backgrounds use a light event tint; detail heroes use the corresponding contrast-safe solid family with inverse text/artwork. Airline dots remain separate. No event-type border is applied to ordinary timeline cards.

Flight and generic Booking pages share hero/schedule/reference hierarchy with type-specific copy. Hotel stays show days/nights and stored room/guest facts. `JourneyEssentials`, `BookingDetailSections`, `BookingCosts`, and `CabStopsManager` keep operational facts, costs, and ordered cab stops available without forcing users through editing. Typed Edit hotel/flight/cab/ferry/train/meal actions live in the top navigation row and remain role-gated. Location-bearing heroes expose navigation. Summary actions are 44px icon targets on phones, with visible text from tablet widths and accessible labels/tooltips throughout. Hotel-host and driver contact names/numbers plus valid Call/WhatsApp actions appear in the opening event modal, not only the booking page.
...
| ----------------------------------------------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Timeline event card                                         | Header expands/collapses; expanded body opens the event sheet | All members may inspect; management remains role-gated and document/navigation/contact controls are independent                                                                                                                                                           |
...
| Edit trip action                                            | Opens Trip settings                                           | Owner only; replaces the removed Trip information overview card                                                                                                                                                                                                           |
...
### 11.5 Root launch and trip selection

`RootRoute` applies `isTripInLaunchWindow()` only at the authenticated root entry. The window runs from start-of-day ten calendar days before departure through end-of-day on the final date in the trip's timezone. Archived/completed trips are excluded. Exactly one eligible trip opens directly; zero or overlapping eligible trips open `/trips` for an explicit choice. Legacy `/home` and `/add` redirect to Trips; direct links and resumed routes are not overridden. The older D-1 focused-trip helper remains for attention resolution, not root-launch routing.

### 11.6 Trips attention and application shell

The primary shell is Trips / Vault / Profile. Trips retains the shared alert engine's highest-priority applicable action as a compact review link, omitting it when there is no urgent action. Readiness links target the specific task sheet. The notification bell opens `NotificationsSheet` without losing the underlying route/query/hash/state, with loading, error, empty, and active grouped states and a link to the full alert-management page.

Mobile header and bottom navigation use compact 56px rows with safe-area handling and labeled touch targets. Only the main header hides while scrolling down; `TripViewTabs` stays sticky and shifts below the main header when it returns. The floating trip dock holds Add event, Everyone/selected traveler, and Jump to current or next. Locally bundled DM Sans and the shared 10/12/16px corner scale apply to UI, not PDF contents.
...
Web Push is gated by `VITE_PUSH_ENABLED`, defaulting off unless enabled for deployment. Profile manages permission, registration/revocation and three device preferences; there is no test-send UI. `push_subscriptions` has owner RLS and column-limited updates; registration uses a validated, ten-device-limited RPC. Browsers cannot access `push_jobs`. Event/cost writes and booking updates queue named change notifications for other active members; claim-time reminders cover explicitly timed events starting within one hour and flights/buses starting in more than four and at most five days. Five-day reminders use a distinct occurrence prefix, expire at departure minus four days, and capture stage/name/departure/time zone in `change_details`; existing one-hour occurrence keys are preserved. Send-time checks revalidate source, membership, preferences, reminder timing/status and early-stage event type. Dates, all-day/inferred times, done/cancelled/skipped events, and task/visa/manual reminders are excluded from timed reminders. The additional `forty_eight_hours` stage covers flights/trains/buses/ferries/cabs more than 24 and at most 48 hours away. Its `reminder:48-hours:` occurrence prefix preserves earlier stage keys, expires at departure minus 24 hours, and uses the same preference and send-time validation. Apply `202609230002_journey_early_reminders.sql` after the change-details migration, then `202609250002_journey_48_hour_reminders.sql`, and redeploy the dispatcher/frontend; fresh setup includes all stages.

`202609230001_push_change_details.sql` adds nullable `push_jobs.change_details` and replaces the change trigger function. New jobs snapshot created/updated/restored action, item/trip titles, changed column names and allowlisted before/after facts. Version, update-audit and sort-only changes are suppressed. Private notes, references, contacts and arbitrary booking JSON values are excluded; their field-change labels can still be summarized. Archiving/deleted source records retain the existing suppression policy; booking inserts still rely on the corresponding event notification rather than a duplicate booking notification. This does not add pushes for document, participant-link, Moment or Cab-stop writes in their own tables.

`push-content.ts` formats up to three changes plus a remainder count, limits title/body to 200/360 characters, respects currency minor-unit precision and formats explicit times using each snapshot's time zone. Untimed ordering timestamps are never presented as appointment times. Dispatch only forwards these summaries after recipient/source revalidation; retries retain the original job tag and snapshot. The worker displays bounded plain text, keeps same-origin destination checks and does not renotify replacements. Pre-migration jobs have null details and retain generic copy; their operation cannot safely be reconstructed. Reminders retain their generic body. Names, amounts and times can appear on lock screens, as Profile discloses. Deploy the migration, `push-dispatch`, then frontend/worker; no live database or device-delivery claim follows from local test success.

`push-dispatch` validates a dedicated scheduler secret; `push-test` validates the user JWT, app Origin, device ownership, and an atomic one-minute rate limit. Server-only VAPID credentials and allowlisted HTTPS endpoints without redirects protect sending. Jobs use occurrence/device uniqueness, five-minute leases, lease-token completion, five attempts, backoff, expiry, and 30-day cleanup. Delivery is at-least-once with stable tags. The generated Workbox worker imports `public/push-worker.js`, preserving offline/update behavior. Taps use `?view=timeline&focus=<event>&notification=<job>`, a booking route, or `?view=details&cost=<cost>&notification=<job>`. Login preserves the URL; timeline navigation reveals/highlights its target and expense links open the expense. Separate Cron SQL uses Vault secrets and runs every minute; applying the migration does not enable it. Offline sync delays changes; OS restrictions and backend pausing prevent exact-time guarantees. [PUSH_SETUP.md](PUSH_SETUP.md) records the pending database, runtime, and device release gates.
...
| ----------------- | --------------------------------------------------------------------------------------------------------------------------- |
| Overview          | Published version, draft availability, release count, section links and collapsed About admin guidance                      |
...
| Booking vendors   | Seller names, aliases, website, public artwork, ordering and enabled state                                                  |
| Journey operators | Read-only bundled train, bus, ferry and cab providers                                                                       |
| Travel defaults   | Read-only stored JSON; no current runtime consumers or browser editing                                                      |
| Appearance        | Allowlisted semantic colour tokens, with actual booking/document component previews for light and dark                      |
| Releases          | On-demand field comparisons, confirmed draft discard, publish, prior versions, audit history and rollback                   |
...
All administrator mutations require a live connection. The console may display last-fetched data when connectivity drops, but editing, saving, asset upload, publishing, rollback and draft deletion are disabled. It has no IndexedDB edit drafts, mutation outbox, background replay, or offline-success message.

`AdminPageHeader` renders one heading and optional essential helper text, without duplicate section eyebrows. `AdminCatalogSearch` keeps a screen-reader label while its visible prompt appears only in the input. Phone catalogue actions use accessible icons; larger screens add labels. `AppearancePreview` shares `BookingHeroSurface` and `DocumentCardContent`; `themePreviewStyle` scopes semantic CSS variables to each preview. Colours controlled by event type remain code-managed. Palette editing is blocked during loading, query failure or offline state, and editors remount when the selected release changes.
...
The September 22 border update increases bundled light/dark line contrast. `readCachedPalette` replaces only exact known legacy/previous-stock token sets; any administrator customization wins. Surface and form borders share the theme's line token.
...
The trip hero uses the primary brand background. In dark mode, `.trip-hero` locally maps `--color-surface` to `--color-canvas`, producing the light-teal background with dark title, metadata, and actions, like the primary search action. It does not replace the brand background with brand-soft. Event detail heroes retain their separate event-color treatment.

`formatMoney()` selects `en-IN` when the currency code is INR (case-insensitive), preserving each currency's minor-unit divisor. Other currencies retain the browser locale. For example, INR 37552500 minor units renders as ₹3,75,525.00; values are formatted, not converted or stored differently.

### 11.13 Reference direction and motion contract

Carousel reference patterns below are design guidance, not a shipped carousel. The active shell and event motion follow sections 11.4–11.6; persistent current state is never inferred from scroll position.
...
| ------------------------- | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------- | ------------------------------------------ |
| Current trip hero         | Readable trip title, dates, context and theme-aware cost action                | No current-trip scale animation in the active Trips-first shell                           | Static layout                              |
| Current timeline/Now card | Semantic border/outline, faint tint and NEXT/NOW or TO DO                      | Three post-arrival focus pulses over 3.5 seconds; separate decorative opening artwork     | Static border and label, no animation      |
...
`/preview` loads a deterministic local-only fixture called **Mediterranean Summer** and is accessible while signed in or out. Its content does not use real trip/Vault APIs; normal authenticated application-shell services are separate. A **Reset demo** action restores the synthetic clock, focus and task state without modifying real data.

`BookingHeroSurface`, `DocumentCardContent`, `CollectionCounts`, `BookingSummarySurface` and the existing cost sheets are reused across demo and production surfaces. `DemoDocumentPreview` fetches only the selected bundled PDF with abortable loading and explicit errors, then uses `DocumentPreview` and `DocumentFileActions` for zoom, download and native sharing. In-app actions say View; the separate device-viewer link says Open. Closing a file or booking cost returns to the underlying event context. `demoEventTimes` supplies dates even for unbooked activities; insurance retains its actual category instead of a flight-icon fallback. The signed-out welcome preview also uses the production event identity and hero styling.
...
| Evening tour waiver                                     | PDF        | Selected members        | Under 250 KB |
...
`EventDocuments` merges explicit event links with inherited booking documents and groups by traveler usage: Everyone first, then traveler order, then unassigned. Existing documents render before smaller Upload new / Attach existing actions; empty states promote creation/attachment. Everyone remains open by default; personal groups can start collapsed in multi-traveler context and reveal the selected traveler. Viewer permissions and document access checks are unchanged. Booking pages retain a single Upload entry point with whole-booking/connection scope where applicable.
...
An itinerary event has a **Documents** section containing zero or more ordered document links. `DocumentCardContent` keeps the full title and type-specific `DocumentTypeIcon` above a compact bottom-left purpose/plain-visibility row; separate bottom-right controls handle ordering and unlinking. File facts and detailed access information remain inside the document page. `DocumentVisibilityIcon` uses Globe for trip-wide, LockKeyhole for private and KeyRound for restricted access. List indicators are passive; detail-page variants expose tap, hover and keyboard explanations without a filled badge. Personal does not override the document type. Timeline document shortcuts omit the visibility control.
...
flowchart TD
    ...
    H --> I{User action}
    I -- View --> J[Use normal local-first document-open flow]
    I -- Unlink --> K[Remove explicit event link or detach booking association]
...
- Ordering is within each traveler section, including inherited booking documents. Focused-traveler groups expand alongside Everyone; collapse state is scoped to the item and traveler.
- Explicit links unlink only from that event. Booking-associated files use online-only `detach_booking_document` to clear the booking/leg association and links from its events atomically; the Vault file and unrelated event links remain. Pending offline edits block this operation until synchronized. Local caches are updated after success, and reattachment clears the link's soft-deletion marker.
...
| ---------------------------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| TypeScript and production build          | 2026-09-23                                                       | Pass, including PWA generation                                                                                                                                                       |
| `npm test`                               | 2026-09-23                                                       | 842 tests across 141 files pass, including reminder-based trip defaults and completed-reminder history                                                                               |
| Complete SQL installer                   | 2026-09-23, isolated PGlite with Supabase Auth/Storage stand-ins | Fresh installation, existing-schema refusal, deferred/admin-gated catalogue publication and safe publication retry pass                                                              |
| SQL smoke scripts 001–007                | 2026-09-23, isolated PGlite                                      | All pass, including document permissions, Planning/Moments, current features and admin draft discard/publication boundaries                                                          |
| HLD / LLD formatting                     | 2026-09-23                                                       | Prettier verification before commit                                                                                                                                                  |
| Browser checks                           | 2026-09-22                                                       | Local demo document preview, responsive sign-in, signed-in admin reads, real release comparison and before/after fields verified; no live configuration mutations                    |
| Hosted SQL / Storage / device acceptance | Separate deployment gates                                        | Apply outstanding incremental migrations after backup; real-device sharing, installed-PWA airplane mode, multi-account permissions and post-push deployment still require acceptance |

The lists below are the release coverage contract, not a claim that every bullet has a dedicated automated test. Local browser reads and isolated PostgreSQL-engine checks do not certify hosted Auth/Storage APIs or deployed-device behaviour. Older test-maintenance counts below are historical; the table above is the current baseline.

September 23 also verifies both standalone push migrations against an existing schema, repeat application, equality with the fresh baseline, captured create/update/restore facts, excluded private values, sender reauthorization, worker fallback/deep links and the early-reminder windows. Local signed-in browser reads verified the Vault event picker and seat disclosures/participant fields in a connected-flight modal. No live document/seat record was changed and no live notification was sent during these tests.

### Test maintenance policy

Test count is not a target. Keep one clear owner for each business rule and add integration coverage only where wiring, permissions, navigation, persistence, or independent failure modes justify it. Preserve meaningful input cases for authorization, uploads/sync, dates/timezones, traveler assignment, and money; do not collapse distinct safety scenarios just to lower the count.

For shared visual components, cover distinct behavioral branches and lifecycle cleanup rather than every event-type × placement × viewport combination. A mocked viewport that the component never reads adds no coverage. Prefer accessible roles/names, links, visible content, state changes, and actions over exact utility classes, padding, or raw CSS text. Keep representative integration checks plus mapping/data tests instead of rerendering every booking type for type-independent logic.

The September 19 audit reduced silhouette tests from 81 to 11 and Booking page tests from 52 to 30, and removed two CSS-only tests. It retained all-type artwork availability and existing security, offline, money, timing, participant, and return-navigation suites. Browser QA remains responsible for actual responsive layout, chevron geometry, border appearance, and the three-pulse/3.5-second visual effect. This cleanup does not claim measured coverage percentages or improved runtime from test count alone.

The follow-up pass reduced the baseline from 704 to 689: ten document-picker checks now run inside existing event-form workflows, two admin smoke cases are covered by their catalog workflows, and launch routing retains the ten-day boundary plus existing active-trip return, overlap, exclusion, and failure cases instead of repeating eligible-date permutations. All eleven event-type document defaults remain asserted; precise calendar/timezone boundaries stay in the presentation suite. Document-row checks use complete accessible names, destinations, and separate actions instead of CSS utilities.

The third pass reduced the baseline from 689 to 674. Exact schedule labels and countdowns share one independently reported presentation test per booking type; hotel-specific and generic page wiring remain covered. Redundant search-alias, document-visibility, synthetic-start, and booking-action cases were removed only where richer tests already checked the same behavior. The retained anchor-loading test checks the blank synthetic start both before and after data arrives. Two modal CSS assertions were replaced with one accessible content-editing and Back-action test. No distinct security, persistence, or timing-validation cases were removed.
...
- Fresh root launch checks the ten-day window, auto-opens exactly one eligible trip, routes overlaps to Trips, and preserves explicit trip links.
...
- Root-launch transitions at ten days before departure and trip-end local boundaries, including overlapping windows
...
3. Select and run only the SQL between `-- PHASE 2 BEGIN: ADMIN-GATED CATALOG PUBLICATION` and `-- PHASE 2 END: ADMIN-GATED CATALOG PUBLICATION`. Do not rerun Phase 1 or the whole setup.
...
| LLD-011 | Root trip selection                            | Root waits for available trips and opens exactly one ten-day-window candidate; zero/overlaps open Trips; retained D-1 helpers do not control root routing                                                                                                                                                                 | Accepted              |
...
| LLD-026 | Browser-editable configuration                 | Airlines, airports, booking vendors, public catalogue assets and semantic light/dark tokens; stored defaults and journey operators are read-only; secrets and security policy remain deployment-managed                                                                                                                   | Accepted              |
| LLD-027 | Configuration lifecycle                        | Online-only draft, comparison, preview, atomic publication, audited discard and rollback; traveler clients cache only a complete published version                                                                                                                                                                        | Accepted              |
...
| LLD-031 | Contextual focus motion                        | Persistent border/tint and explicit label distinguish current entry; one-shot landing/artwork animations honor reduced motion without continuous scale or layout shift                                                                                                                                                    | Accepted              |
...
| LLD-057 | Admin UI refresh                               | Responsive console with concise headings, compact actions, shared-component theme previews, field-level release changes and confirmed draft discard                                                                                                                                                                       | Implemented           |
...
| LLD-064 | Booking-vendor fallback                        | Keep searchable pickers available after Other, reconcile vendor URLs, bundle Airbnb/Trip.com and publish via the canonical SQL catalogue phase without rewriting booking snapshots                                                                                                                                        | Accepted              |
...
| LLD-080 | Unified timeline filters                       | Retire TripAgendaSheet; use TripTimeline date groups and TimelineFilters with draft date/type/display changes and profile/trip/traveler session state                                                                                                                                                                     | Accepted              |
...
| LLD-082 | Focus placement and pulse                      | Center the target heading in usable viewport; wait for scroll settlement; three stronger pulses over 3500ms with semantic current colors; reduced motion disables animation                                                                                                                                               | Accepted              |
| LLD-083 | Event document grouping                        | Render existing documents before secondary add actions; Everyone first/open, traveler groups follow with contextual expansion, unassigned last; no manual ordering controls                                                                                                                                               | Accepted              |
...
### September 19 implementation decisions

| ID      | Topic                   | Decision                                                                                                                                                        | Status                                 |
| ------- | ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| LLD-088 | Contextual return state | Explicit nested origin navigation plus card-offset restoration; new target/current/search intents clear stale restoration                                       | Accepted                               |
| LLD-089 | Event artwork           | Compact header icons; bottom-right one-shot silhouettes in expanded summaries, modals, and typed heroes; 80% summary / 50% other placements                     | Accepted                               |
| LLD-090 | Money and trip hero     | INR uses en-IN grouping; dark trip hero uses brand background and canvas-colored content                                                                        | Accepted                               |
| LLD-091 | Push infrastructure     | Owner-scoped devices, protected jobs/sender, opt-in timed reminders, generic payloads and exact record links; authenticated delivery/device checks remain gated | Functions deployed; acceptance pending |
...
The complete setup includes `delete_archived_trip_item(trip_id, item_id, kind)`. The RPC checks authenticated trip edit permission, a same-trip archived record, and an allowlisted kind. Booking deletion locks/checks its grouped events; any relative scheduling dependency blocks the transaction. Linked cost and document associations are detached before deleting events/bookings, retaining their records and file bytes. This mutation is never queued offline. Archive mutation success invalidates relevant trip collections and alerts. For existing deployments, use only reviewed missing sections; installing the RPC does not delete rows.
...
| Fresh-launch router                           | `src/components/RootRoute.tsx`                                                                                                                                                                                                                                                        | Ten-day trip window, exactly-one auto-open, explicit overlap selection in Trips                                                                                                                                          |
...
| Database deltas                               | `supabase/TRIP_VAULT_COMPLETE_SETUP.sql`                                                                                                                                                                                                                                              | Complete schema, functions, grants, RLS and current-feature setup                                                                                                                                                        |
...
| Account-document Storage hardening            | `supabase/TRIP_VAULT_COMPLETE_SETUP.sql`                                                                                                                                                                                                                                              | Verified completion, append-only Storage policies, association immutability, and unassociated cleanup boundary                                                                                                           |
| Booking-vendor catalog addition               | `supabase/TRIP_VAULT_COMPLETE_SETUP.sql`                                                                                                                                                                                                                                              | Copies the published release and adds Airbnb and Trip.com                                                                                                                                                                |
| Trip Storage cleanup queue                    | `supabase/TRIP_VAULT_COMPLETE_SETUP.sql`                                                                                                                                                                                                                                              | Persistent owner cleanup work, guarded legacy-object deletion, permanent-delete RPC, and server-hardened flight connection append                                                                                        |
| Relative-event timing migration               | `supabase/TRIP_VAULT_COMPLETE_SETUP.sql`                                                                                                                                                                                                                                              | Explicit-start marker, optional planned duration, derivation constraints, anchor fallback propagation, and trip bounds                                                                                                   |
| Event-form data-model migration               | `supabase/TRIP_VAULT_COMPLETE_SETUP.sql`                                                                                                                                                                                                                                              | Reservation state, participant scope, optional non-flight arrival, typed ground details, per-leg traveler allocations, and atomic hotel-stay save                                                                        |
| Journey placement/timezone migration          | `supabase/TRIP_VAULT_COMPLETE_SETUP.sql`                                                                                                                                                                                                                                              | Journey placement editing and Domestic journey/connected-flight timezone updates                                                                                                                                         |
| Cab-stop migration                            | `supabase/TRIP_VAULT_COMPLETE_SETUP.sql`                                                                                                                                                                                                                                              | Versioned ordered stops, same-trip event links, optional cost links, indexes, triggers, grants, and RLS                                                                                                                  |
| Timeline filters and focus                    | `src/features/timeline/TimelineFilters.tsx`, `src/features/timeline/model.ts`, `src/features/timeline/scroll.ts`                                                                                                                                                                      | Draft date/type/display filters, date/type aliases, usable-viewport centering and two-pulse feedback                                                                                                                     |
