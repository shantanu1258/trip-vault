...
> **Current contract:** Reconciled with implementation through `7cdd785`; see the [changelog](../CHANGELOG.md). Earlier review/amendment notes are historical. Web Push below is a proposal, not deployed behavior.
...
Web Push and scheduled server delivery are **proposed, not implemented or enabled**. The repository currently has no push-subscription migration, push sender, or server reminder job. The optional design is: per-device opt-in → authenticated subscription storage with owner-scoped RLS → protected scheduled Edge Function → browser push service → service-worker notification → event deep link. Use server-held VAPID private credentials, a deliberate permission action, device revocation, expiry, deduplication/retries, and authorization checks at send time. Recalculate/cancel scheduled reminders when events change or tasks complete; offline edits affect server scheduling only after sync. Never expose service credentials or permit unauthenticated arbitrary sends. A future implementation must extend the existing generated PWA worker without losing caching/update behavior.

The proposed scheduler can use Supabase Cron (`pg_cron`) and HTTP calls (`pg_net`) to an Edge Function, with scheduler credentials in Vault and function secrets server-side. Enabling extensions alone does not implement notifications. Offline devices cannot receive new push; a push service may retain messages only within their expiry, with no exact-time guarantee. HTTPS and browser permission are required; iOS/iPadOS Web Push requires a supported installed Home Screen app. Free-tier backend pausing is a delivery risk; quotas/pricing must be rechecked before deployment, not treated as a permanent architecture guarantee. In-app alerts continue independently.
...
| `npm test -- --maxWorkers=4 --minWorkers=1`                    | 2026-09-19, documentation reconciliation                                                                                                                                 | Pass: 798 tests across 102 files; older failing selector/timeout notes are superseded by this completed run |
...
| LLD-091 | Push infrastructure     | Device subscriptions, protected scheduled sender, VAPID secrets, expiration and deduplication remain a separate proposed implementation     | Proposed |
