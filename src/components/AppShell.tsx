...
export function AppShell({ children, demo = false }: { children: ReactNode; demo?: boolean }) {
  ...
  return (
    <div className="min-h-dvh bg-canvas text-ink">
        ...
        <div className="mx-auto flex h-[4.75rem] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <NavLink
              ...
              className={({ isActive }) => `tap-target relative grid size-11 place-items-center rounded-2xl border border-line bg-surface transition-colors hover:border-brand/40 ${isActive ? "text-brand" : "text-muted"}`}
            >
              <Bell className="size-5" aria-hidden="true" />
              {alertCount > 0 && <span className="absolute right-1.5 top-1 grid min-w-4 place-items-center rounded-full bg-coral px-1 text-[0.55rem] font-black text-white" aria-hidden="true">{alertCount > 9 ? "9+" : alertCount}</span>}
            </NavLink>
            <ThemeToggle />
          </div>
        </div>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-line bg-surface/95 px-2 pb-[max(0.55rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur-md lg:hidden" aria-label="Primary navigation">
