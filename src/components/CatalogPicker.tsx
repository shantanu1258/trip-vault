...
export function CatalogPicker({
  ...
  const results = useMemo(() => {
    ...
    return options
      .filter((option) =>
        `${option.title} ${option.detail} ${option.searchText ?? ""}`
          .toLocaleLowerCase()
          .includes(normalized)
      )
      .slice(0, 100);
            ...
            >
              <div className="flex items-center justify-between gap-3 px-1 pb-3">
                <div>
                  ...
                  <h2 className="font-display text-lg font-black">{label}</h2>
                </div>
                <button
                  type="button"
              ...
              >
                  ...
                  return (
                    ...
                    >
                      <span className="grid size-9 shrink-0 place-items-center rounded-xl bg-elevated text-brand">
                        {active ? <Check className="size-4" /> : <Search className="size-4" />}
                      </span>
                      <span className="min-w-0 flex-1">
                        <strong className="block truncate text-sm">{option.title}</strong>
                        <span className="block text-[.68rem] leading-5 text-muted">
                          {option.detail}
                        </span>
                      </span>
                    </button>
                ...
                {!results.length && (
                  <p className="p-5 text-center text-sm text-muted">
                    No saved option matches that search.
                  </p>
                )}
                <button
                  type="button"
                  ...
                  className="flex min-h-16 w-full items-center gap-3 border-t border-line bg-elevated px-3 py-3 text-left text-brand"
                >
                  <span className="grid size-9 shrink-0 place-items-center rounded-xl bg-brand-soft font-black">
                    +
                  </span>
                  <span>
                    <strong className="block text-sm">{otherLabel}</strong>
                    <span className="block text-[.68rem] text-muted">
                      Enter it manually; an admin can review it later.
                    </span>
                  </span>
                </button>
              </div>
