...
export function FocusSurface({ active, children, className = "" }: { active: boolean; children: ReactNode; className?: string }) {
  return (
    <article
      ...
      className={`surface-card relative transform-gpu overflow-hidden transition-[transform,box-shadow,border-color] duration-200 ease-settle motion-reduce:transition-none ${
        active ? "z-10 scale-[1.015] border-coral/70 shadow-focus motion-reduce:scale-100" : "scale-100"
      } ${className}`}
    >
