...
export function FileDropzone({
  ...
  busy = false,
  compact = false,
  description = "PDF, JPEG, PNG, or WebP under 5 MB",
  onValidationError
}: FileDropzoneProps) {
  const inputId = useId();
  ...
  return (
    <div className="min-w-0">
      ...
      >
        <span
          className={`flex min-w-0 ${compact ? "items-center gap-3 text-left" : "flex-col items-center"}`}
        >
          <span
            className={`${compact ? "grid size-9" : "grid size-12"} shrink-0 place-items-center rounded-2xl ${file ? "bg-success/10 text-success" : "bg-brand-soft text-brand"}`}
          >
            {busy ? (
          ...
          <span className="min-w-0">
            <strong className={`${compact ? "truncate" : "mt-3"} block text-sm`}>
              {busy ? "Saving file…" : file ? file.name : (prompt ?? label)}
            </strong>
            <span
              id={`${inputId}-description`}
              className={`${compact ? "mt-0.5" : "mt-1"} block text-xs font-medium text-muted`}
            >
              {file
                ? `${(file.size / 1_000_000).toFixed(2)} MB · Select to choose a different file`
                : `${description}${compact ? "" : " · tap or drop it here"}`}
            </span>
          </span>
        </span>
      </button>
      ...
      {validationError && (
        <p id={`${inputId}-error`} role="alert" className="mt-2 text-xs font-bold text-danger">
          {validationError}
        </p>
      )}
    </div>
