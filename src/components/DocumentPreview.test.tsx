...
describe("DocumentPreview", () => {
  ...
  it("renders PDF pages on a canvas with page and zoom controls", async () => {
    ...
    const view = render(<DocumentPreview blob={blob} url="blob:ticket" title="Train ticket" mimeType="application/pdf" filename="ticket.pdf" pdfModuleLoader={loader} />);
  ...
  it("provides in-app image zoom without turning the preview into an external link", async () => {
    const user = userEvent.setup();
    render(<DocumentPreview blob={new Blob(["image"], { type: "image/png" })} url="blob:photo" title="Hotel voucher" mimeType="image/png" filename="voucher.png" />);
