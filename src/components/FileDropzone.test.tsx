...
describe("FileDropzone", () => {
  it("uses a large centered picker and reports the selected file", async () => {
    ...
    const view = render(<FileDropzone name="file" label="Travel document" prompt="Choose the PDF or image" file={null} onFileChange={onFileChange} />);

    expect(screen.getByRole("button", { name: /choose the pdf or image/i })).toHaveClass("min-h-32");
    await user.upload(screen.getByLabelText("Travel document"), file);
    expect(onFileChange).toHaveBeenCalledWith(file);
    view.rerender(<FileDropzone name="file" label="Travel document" prompt="Choose the PDF or image" file={file} onFileChange={onFileChange} />);
  ...
  it("accepts drag and drop", () => {
    ...
    render(<FileDropzone name="file" label="Travel document" file={null} onFileChange={onFileChange} />);

    fireEvent.drop(screen.getByRole("button", { name: /travel document/i }), { dataTransfer: { files: [file] } });
    expect(onFileChange).toHaveBeenCalledWith(file);
  });
  ...
  it("clears the native picker before opening so the same file can be selected again", async () => {
    ...
    Object.defineProperty(input, "value", { configurable: true, writable: true, value: "C:\\fakepath\\ticket.pdf" });
  ...
  it("rejects unsupported files before submission", () => {
    ...
    fireEvent.change(screen.getByLabelText("Travel document"), { target: { files: [new File(["x"], "page.html", { type: "text/html" })] } });
  ...
  it("rejects a file at the five-megabyte service limit", () => {
    const file = new File([new Uint8Array(5_000_000)], "large-ticket.pdf", { type: "application/pdf" });
    expect(() => prepareDocumentFile(file)).toThrow("Choose a file smaller than 5 MB");
  });

  it("disables selection and announces a busy save", () => {
    render(<FileDropzone name="file" label="Travel document" file={new File(["x"], "ticket.pdf", { type: "application/pdf" })} onFileChange={vi.fn()} busy />);
