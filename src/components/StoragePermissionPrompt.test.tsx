...
vi.mock("../features/sync/localSync", () => ({ localProfileId: vi.fn().mockResolvedValue("profile-1") }));
...
describe("offline storage request", () => {
  ...
  it("asks after authentication and requests persistence from a user action", async () => {
    ...
    render(<MemoryRouter><StoragePermissionPrompt /></MemoryRouter>);

    expect(await screen.findByRole("heading", { name: /keep trip files on this device/i })).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: /allow device storage/i }));

    expect(persist).toHaveBeenCalledOnce();
    await waitFor(() => expect(screen.queryByRole("heading", { name: /keep trip files/i })).not.toBeInTheDocument());
  });
