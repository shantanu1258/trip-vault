...
function supportedTimeZones() {
  ...
  return [...new Set([...commonTimeZones, ...available])]
...
export function TimeZoneAutocomplete({
  ...
  const initialValue =
    defaultValue && isSupportedTimeZone(defaultValue)
      ? defaultValue
      : requireSelection
        ? ""
        : deviceTimeZone;
  ...
  const options = useMemo(() => {
    const available =
      !selected || allTimeZones.includes(selected) ? allTimeZones : [selected, ...allTimeZones];
    const normalized = query.trim().toLocaleLowerCase();
    const matches = available.filter((timeZone) => {
      if (!normalized) return true;
      const details = zoneDetails(timeZone);
      return `${timeZone} ${details.city} ${details.region} ${details.offset}`
        .toLocaleLowerCase()
        .includes(normalized);
    });
    if (!normalized) {
      const preferred = [
        ...new Set([selected, deviceTimeZone, ...commonTimeZones].filter(Boolean))
      ];
      matches.sort((a, b) => {
  ...
  useEffect(() => {
    ...
    const place = () => {
      ...
      if (!isMobile && triggerRef.current) {
        ...
        setPosition({
          top: Math.max(12, Math.min(rect.bottom + 8, window.innerHeight - 472)),
    ...
    <div className="relative">
      <input
        ...
        onInput={(event) => {
          ...
          if (isSupportedTimeZone(value)) setSelected(value);
        }}
      />
      <button
        ref={triggerRef}
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <strong className="block truncate text-sm">
            {selectedDetails?.city ?? "Choose time zone"}
          </strong>
          <span className="block truncate text-[.68rem] font-medium text-muted">
            {selectedDetails
              ? `${selected} · ${selectedDetails.offset}`
              : "Required for this international airport"}
          </span>
        </span>
          ...
          >
            <section
              ...
              style={
                mobile
                  ? {
                      top: viewport.top + 8,
                      height: Math.max(120, viewport.height - 16),
                      maxHeight: Math.max(120, viewport.height - 16)
                    }
            ...
            >
              <div className="flex items-center justify-between gap-3 px-1 pb-3">
                <div>
                  <p className="eyebrow">Local clock</p>
                  <h2 className="font-display text-lg font-black">Choose time zone</h2>
                </div>
                <button
                  type="button"
                  className="tap-target grid size-10 place-items-center rounded-full border border-line"
                  onClick={() => setOpen(false)}
                  aria-label="Close time zone picker"
                >
                  <X className="size-4" />
                </button>
              </div>
              <label className="relative block">
                ...
                <input
                  ...
                  placeholder="Search city, region, UTC offset…"
                  autoComplete="off"
                />
              </label>
              <div
              ...
              >
                {options.map((timeZone) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex min-h-16 w-full items-center gap-3 border-b border-line px-3 py-2 text-left last:border-0 hover:bg-brand-soft ${active ? "bg-brand-soft" : "bg-surface"}`}
                    >
                      <span className="grid size-9 shrink-0 place-items-center rounded-xl bg-elevated text-brand">
                        {active ? <Check className="size-4" /> : <Clock3 className="size-4" />}
                      </span>
                      <span className="min-w-0 flex-1">
                        <strong className="block truncate text-sm">{details.city}</strong>
                        <span className="block truncate text-[.68rem] text-muted">
                          {details.region} · {timeZone}
                        </span>
                      </span>
                      <span className="shrink-0 text-right">
                        <strong className="block text-xs">{details.offset}</strong>
                        <span className="block text-[.65rem] text-muted">{details.localTime}</span>
                      </span>
                    </button>
                  );
                })}
                {!options.length && (
                  <p className="p-6 text-center text-sm text-muted">
                    No supported time zone matches that search.
                  </p>
                )}
              </div>
              <p className="px-1 pt-3 text-[.68rem] leading-5 text-muted">
                Times are stored using the selected IANA zone so daylight-saving changes remain
                correct.
              </p>
            </section>
          </div>,
