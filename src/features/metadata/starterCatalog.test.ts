...
describe("bundled travel catalogues", () => {
  ...
  it("covers prominent Indian and regional airports used by the target trips", () => {
    ...
    expect(["DEL", "BOM", "BLR", "HYD", "MAA", "CCU", "COK", "GOI", "GOX", "PNQ", "GAU", "LKO", "JAI", "TRV", "CCJ", "IXC", "SXR", "ATQ", "VNS", "NAG"].every((code) => codes.has(code))).toBe(true);
    expect(["SIN", "KUL", "PEN", "LGK", "DPS", "CGK", "SUB", "YIA", "LOP", "LBJ"].every((code) => codes.has(code))).toBe(true);
  });
  ...
  it("keeps airline identifiers unique and external actions safe", () => {
    ...
    for (const airline of airlines) {
      ...
      expect([airline.checkInUrlTemplate, airline.manageBookingUrlTemplate, airline.statusUrlTemplate, airline.trackerUrlTemplate].every((url) => url === null || validateActionUrl(url))).toBe(true);
    }
  });
