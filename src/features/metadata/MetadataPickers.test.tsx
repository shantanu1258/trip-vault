...
vi.mock("./publishedConfig", () => ({
  listAvailableAirlines: vi.fn().mockResolvedValue([
    {
      stableKey: "air-india",
      name: "Air India",
      iataCode: "AI",
...
describe("travel metadata pickers", () => {
  it("chooses a saved airline or exposes a manual Other field", async () => {
    ...
    expect(
      screen.getByPlaceholderText("Search airline name, IATA code, or ICAO code")
    ).toBeInTheDocument();
    await user.click(await screen.findByRole("option", { name: /Air India AI · AIC/i }));
    expect(container.querySelector<HTMLInputElement>('input[name="airline"]')?.value).toBe(
      "Air India"
    );
    expect(container.querySelector<HTMLInputElement>('input[name="airlineSource"]')?.value).toBe(
      "catalog"
    );
    ...
    expect(
      screen.getByPlaceholderText("Enter the airline exactly as shown on the ticket")
    ).toBeRequired();
    expect(container.querySelector<HTMLInputElement>('input[name="airlineSource"]')?.value).toBe(
      "other"
    );
  ...
  it("derives airport code, country, and time zone from one selection", async () => {
    const { user, container } = renderPicker(
      <AirportPicker
        name="departureName"
        codeName="departureCode"
        countryName="departureCountry"
        timezoneName="departureTimezone"
        label="From airport"
    ...
    expect(
      screen.getByPlaceholderText("Search airport name, city, or IATA code")
    ).toBeInTheDocument();
    await user.click(
      await screen.findByRole("option", { name: /DEL · Indira Gandhi International Airport/i })
    );
    ...
    expect(container.querySelector<HTMLInputElement>('input[name="departureName"]')?.value).toBe(
      "Indira Gandhi International Airport"
    );
    expect(container.querySelector<HTMLInputElement>('input[name="departureCountry"]')?.value).toBe(
      "IN"
    );
    expect(
      container.querySelector<HTMLInputElement>('input[name="departureTimezone"]')?.value
    ).toBe("Asia/Kolkata");

    await user.click(screen.getByRole("button", { name: "Choose from airport" }));
    await user.click(screen.getByRole("option", { name: /Other from airport/i }));
    expect(
      screen.getByPlaceholderText("Enter the full airport name printed on the ticket")
    ).toBeRequired();
    expect(screen.getByPlaceholderText("Enter the 3-letter IATA code")).toHaveAttribute(
      "maxlength",
      "3"
    );
    ...
    expect(
      container.querySelector<HTMLInputElement>('input[name="departureNameSource"]')?.value
    ).toBe("other");
  });
  ...
  it("filters a domestic destination to the origin country and keeps its fallback time zone hidden", async () => {
    ...
    expect(
      screen.queryByRole("option", { name: /DXB · Dubai International Airport/i })
    ...
    expect(
      screen.queryByPlaceholderText("Enter the 2-letter country code")
    ).not.toBeInTheDocument();
    expect(container.querySelector<HTMLInputElement>('input[name="arrivalCountry"]')?.value).toBe(
      "IN"
    );
    expect(container.querySelector<HTMLInputElement>('input[name="arrivalTimezone"]')?.value).toBe(
      "Asia/Kolkata"
    );
  });
  ...
  it("fills the known booking website and supports an admin-reviewable Other source", async () => {
    ...
    expect(
      screen.getByPlaceholderText("Search booking website, seller, or travel agent")
    ).toBeInTheDocument();
    ...
    expect(container.querySelector<HTMLInputElement>('input[name="bookedViaName"]')?.value).toBe(
      "Cleartrip"
    );
    ...
    await user.type(
      screen.getByPlaceholderText("Enter the website, agent, or business that sold the booking"),
      "Local agent"
    );
    await waitFor(() =>
      expect(
        container.querySelector<HTMLInputElement>('input[name="bookedViaNameSource"]')?.value
      ).toBe("other")
    );
    ...
    await user.click(await screen.findByRole("option", { name: /Cleartrip/i }));
    expect(
      screen.queryByPlaceholderText("Enter the website, agent, or business that sold the booking")
    ).not.toBeInTheDocument();
    expect(container.querySelector<HTMLInputElement>('input[name="bookedViaName"]')?.value).toBe(
      "Cleartrip"
    );
  ...
  it("offers mode-specific journey operators and keeps Other reversible", async () => {
    const { user, container } = renderPicker(
      <JourneyOperatorPicker mode="bus" name="operator" required />
    );
    ...
    expect(container.querySelector<HTMLInputElement>('input[name="operator"]')?.value).toBe(
      "Qistna Express"
    );
    expect(container.querySelector<HTMLInputElement>('input[name="operatorSource"]')?.value).toBe(
      "catalog"
    );
    ...
    expect(
      screen.queryByPlaceholderText("Enter the bus operator shown on the ticket")
    ).not.toBeInTheDocument();
    expect(container.querySelector<HTMLInputElement>('input[name="operator"]')?.value).toBe(
      "Causeway Link"
    );
  ...
  it("offers regional cab providers while preserving a manual local-cab option", async () => {
    ...
    expect(
      screen.getByPlaceholderText("Enter the cab operator shown on the ticket")
    ).not.toBeRequired();
    expect(container.querySelector<HTMLInputElement>('input[name="operatorSource"]')?.value).toBe(
      "other"
    );
