...
function ScheduleFields({ startDefault, endDefault, durationMinutes, startRequired, trip }: { startDefault: string; endDefault: string; durationMinutes?: number | null; startRequired: boolean; trip: Trip }) {
  ...
  return <div className="mt-4 grid gap-4 sm:grid-cols-2">
    <label className="form-label">{startRequired ? "Starts" : "Start date & time (optional)"}<input className="form-input" name="startsAt" type="datetime-local" min={`${trip.start_date}T00:00`} max={`${trip.end_date}T23:59`} value={start} onChange={(event) => setStart(event.target.value)} required={startRequired} /></label>
...
export function TimingFields({ trip, itinerary, item }: { trip: Trip; itinerary: ItineraryItem[]; item?: ItineraryItem }) {
  ...
  return <fieldset className="rounded-2xl border border-line p-4">
    ...
    {mode === "relative" && <>
      <div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="form-label">Position<select className="form-input" name="relativePosition" defaultValue={item?.relative_position ?? "after"}><option value="before">Before</option><option value="after">After</option></select></label><label className="form-label">Event<select className="form-input" name="anchorItineraryItemId" defaultValue={item?.anchor_itinerary_item_id ?? ""} required><option value="">Choose a dated event</option>{itinerary.filter((candidate) => candidate.id !== item?.id && !["relative", "unscheduled"].includes(candidate.timing_mode ?? "exact")).map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.title}</option>)}</select></label></div>
