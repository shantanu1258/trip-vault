...
export function TimingFields({ trip, itinerary, item }: { trip: Trip; itinerary: ItineraryItem[]; item?: ItineraryItem }) {
  ...
  return <fieldset className="rounded-2xl border border-line p-4">
    ...
    {mode === "relative" && <>
      <div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="form-label">Position<select className="form-input" name="relativePosition" value={relativePosition} onChange={(event) => setRelativePosition(event.target.value as "before" | "after")}><option value="before">Before</option><option value="after">After</option></select></label><label className="form-label">Event<select className="form-input" name="anchorItineraryItemId" value={anchorItineraryItemId} onChange={(event) => setAnchorItineraryItemId(event.target.value)} required><option value="">Choose a dated event</option>{itinerary.filter((candidate) => candidate.id !== item?.id && !["relative", "unscheduled"].includes(candidate.timing_mode ?? "exact")).map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.title}</option>)}</select></label></div>
