...
export function suggestedFlightLocalTime(
  ...
  try {
    ...
    return isoToLocalDateTime(
      new Date(Date.parse(instant) + minutesLater * 60_000).toISOString(),
      toTimeZone
    );
  } catch {
    return "";
...
function OccurrenceSelect({ name }: { name: string }) {
  return (
    <select
      ...
      aria-label="Repeated clock time choice"
    >
...
function RepeatedClockHelp() {
  return (
    <p className="mt-2 text-xs leading-5 text-muted">
      Use Automatic normally. Choose an occurrence only if daylight saving makes the printed local
      time happen twice.
    </p>
  );
}

function endpointLabel(code: string | null | undefined, name: string | null | undefined) {
...
function JourneyLegCard({
  ...
  return (
    <details
      ...
      className="group rounded-2xl border border-line p-4"
    >
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 [&::-webkit-details-marker]:hidden">
        <span className="min-w-0">
          <strong className="block font-display text-lg font-black capitalize">{title}</strong>
...
export function JourneyScopeFields({
  value,
  onChange
}: {
  value: JourneyScope;
  onChange: (value: JourneyScope) => void;
}) {
  return (
    <fieldset className="grid grid-cols-2 gap-3 rounded-2xl border border-line p-4">
      <legend className="px-1 text-sm font-extrabold">Journey type</legend>
      <label
        className={`flex items-center gap-2 rounded-xl border p-3 text-sm font-bold ${value === "domestic" ? "border-brand bg-brand-soft" : "border-line bg-elevated"}`}
      >
        <input
          type="radio"
          name="journeyScope"
      ...
      <label
        className={`flex items-center gap-2 rounded-xl border p-3 text-sm font-bold ${value === "international" ? "border-brand bg-brand-soft" : "border-line bg-elevated"}`}
      >
        <input
          type="radio"
          name="journeyScope"
...
export function RouteStructureFields({
  mode,
  value,
  onChange
}: {
  mode: "flight" | Exclude<JourneyMode, "cab">;
  value: RouteStructure;
  onChange: (value: RouteStructure) => void;
}) {
  const noun = mode === "flight" ? "flight" : mode === "ferry" ? "sailing" : mode;
  return (
    <fieldset className="grid grid-cols-2 gap-3 rounded-2xl border border-line p-4">
      <legend className="px-1 text-sm font-extrabold">Route</legend>
      <label
        className={`flex items-center gap-2 rounded-xl border p-3 text-sm font-bold ${value === "direct" ? "border-brand bg-brand-soft" : "border-line bg-elevated"}`}
      >
        <input
          type="radio"
        ...
        />{" "}
        {mode === "flight" ? "Direct" : `Single ${noun}`}
      </label>
      <label
        className={`flex items-center gap-2 rounded-xl border p-3 text-sm font-bold ${value === "connecting" ? "border-brand bg-brand-soft" : "border-line bg-elevated"}`}
      >
        <input
          type="radio"
          name="journeyStructure"
...
function TravelerAllocationFields({
  prefix,
  travelers,
  mode,
  showSeatAndCabin = true
}: {
  prefix: string;
  travelers: Traveler[];
  mode: "flight" | JourneyMode;
  showSeatAndCabin?: boolean;
}) {
  ...
  return (
    <details className="mt-4 rounded-xl border border-line/80 p-3">
      ...
      <p className="mt-2 text-xs leading-5 text-muted">
        Only fill details already shown on the ticket. They can be updated later.
      </p>
            ...
            <div className="mt-2 grid gap-3 sm:grid-cols-3">
              {showSeatAndCabin && (
                <label className="form-label text-xs">
                  ...
                  <input
                    className="form-input"
                    name={`${prefix}.traveler.${traveler.id}.seat`}
                    placeholder={
                      mode === "ferry"
                        ? "Enter the assigned seat"
                        : "Enter the seat or berth, if assigned"
                    }
                  />
                </label>
              )}
              {mode === "flight" && (
                <label className="form-label text-xs">
                  ...
                  <input
                    className="form-input"
                    name={`${prefix}.traveler.${traveler.id}.group`}
                    placeholder="Enter the boarding group, if provided"
                  />
                </label>
              )}
              <label className="form-label text-xs">
                {mode === "flight" ? "Passenger ticket number" : "Passenger reference"}
                <input
                  className="form-input"
                  name={`${prefix}.traveler.${traveler.id}.reference`}
                  placeholder="Enter the passenger-specific reference, if provided"
                />
              </label>
              {mode === "train" && (
                <label className="form-label text-xs">
                  ...
                  <input
                    className="form-input"
                    name={`${prefix}.traveler.${traveler.id}.coach`}
                    placeholder="Enter the coach, if provided"
                  />
                </label>
              )}
              {mode === "ferry" && showSeatAndCabin && (
                <label className="form-label text-xs">
                  Cabin
                  <input
                    className="form-input"
                    name={`${prefix}.traveler.${traveler.id}.coach`}
                    placeholder="Enter the cabin, if provided"
                  />
                </label>
              )}
            </div>
...
export function FlightLegFields({
  ...
  direct: boolean;
  removable: boolean;
  previousLeg?: FlightLegDraft;
  onArrivalChange?: (value: FlightLegDraft) => void;
  onRemove: () => void;
}) {
  const prefix = `flight.${index}`;
  const connectionDeparture = previousLeg?.arrivalAirport
    ? suggestedFlightLocalTime(
        previousLeg.arrivalLocal,
        previousLeg.arrivalAirport.timezone,
        previousLeg.arrivalAirport.timezone,
        120
      )
    : "";
  ...
  const [departureAirport, setDepartureAirport] = useState<AvailableAirport | null>(
    previousLeg?.arrivalAirport ?? null
  );
  ...
  const calculatedBoarding = boardingLead
    ...
    : "";
  const departureLabel = endpointLabel(
    departureAirport?.iataCode ?? departureAirport?.icaoCode,
    departureAirport?.name
  );
  const arrivalLabel = endpointLabel(
  ...
  useEffect(() => {
    onArrivalChange?.({ arrivalAirport, arrivalLocal: arrival });
    ...
    >
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="form-label">
          Airline
          <RequiredMark />
          <AirlinePicker name={`${prefix}.airline`} />
        </label>
        <label className="form-label">
          Flight number
          <RequiredMark />
          <input
            ...
            name={`${prefix}.number`}
            placeholder="Enter the number printed on the ticket"
            required
          />
        </label>
      </div>
      <div className="mt-4">
        <AirportPicker
          name={`${prefix}.departureName`}
          codeName={`${prefix}.departureCode`}
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        {international ? (
          <label className="form-label">
            ...
            <OccurrenceSelect name={`${prefix}.departureOccurrence`} />
          </label>
        ) : (
          <input type="hidden" name={`${prefix}.departureOccurrence`} value="earlier" />
        )}
      </div>
      <div className="mt-4">
        <AirportPicker
          name={`${prefix}.arrivalName`}
          codeName={`${prefix}.arrivalCode`}
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        {international ? (
          <label className="form-label">
            ...
            <OccurrenceSelect name={`${prefix}.arrivalOccurrence`} />
          </label>
        ) : (
        ...
        <div className="mt-3 grid gap-4 sm:grid-cols-3">
          <label className="form-label">
            ...
            <input
              ...
              placeholder="Enter minutes before departure"
            />
          </label>
          <label className="form-label">
          ...
          {international ? (
            <label className="form-label">
              ...
              <OccurrenceSelect name={`${prefix}.boardingOccurrence`} />
            </label>
          ) : (
            <input type="hidden" name={`${prefix}.boardingOccurrence`} value="earlier" />
          )}
          <label className="form-label">
            Departure terminal
            <input
              ...
              placeholder="Enter the terminal shown on the ticket"
            />
          </label>
          <label className="form-label">
            Departure gate
            <input
              ...
              placeholder="Enter the gate when known"
            />
          </label>
          <label className="form-label">
            Arrival terminal
            <input
              ...
              placeholder="Enter the arrival terminal when known"
            />
          </label>
        </div>
        {calculatedBoarding && (
          <p className="mt-3 rounded-xl bg-brand-soft px-3 py-2 text-xs font-bold text-brand">
            ...
            airport. An exact time overrides this.
          </p>
        )}
...
const routeLabels: Record<
  ...
  { origin: string; destination: string; operator: string; service: string }
> = {
  train: {
...
export function GroundJourneyLegFields({
  index,
  mode,
  trip,
  defaultTimezone,
  ...
  const calculatedBoarding = boardingLead
    ? localDateTimeMinusMinutes(departure, Number(boardingLead))
    : "";
  const booked = reservationState === "booked";
    ...
    >
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="form-label">
          ...
          <JourneyOperatorPicker mode={mode} name={`${prefix}.operator`} required={booked} />
        </div>
        <label className="form-label">
          {labels.service} (optional)
          <input
            className="form-input"
            name={`${prefix}.service`}
            placeholder={
              booked
                ? "Enter the number printed on the ticket, if provided"
                : "Add the service number later if it is not known yet"
            }
          />
        </label>
      </div>
      {mode === "train" && (
        <label className="form-label mt-4">
          ...
          <input
            className="form-input"
            name={`${prefix}.trainName`}
            placeholder="Enter the train name if it is already known"
          />
        </label>
      )}
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
          ...
          <input
            ...
            placeholder={
              mode === "train"
                ...
                : mode === "bus"
                  ? "Enter the pickup point printed on the ticket"
                  : "Search for the departure terminal or pier"
            }
            required
          />
        </label>
        <label className="form-label">
          {labels.origin} code (optional)
          <input
            className="form-input uppercase"
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        <label className="form-label">
          ...
          <input
            className="form-input"
            name={`${prefix}.destinationName`}
            onChange={(event) => setDestinationName(event.target.value)}
            placeholder={
              mode === "train"
                ? "Search by station name or code"
                : mode === "bus"
                  ? "Enter the drop-off point printed on the ticket"
                  : "Search for the arrival terminal or pier"
            }
            required
          />
        </label>
        <label className="form-label">
          {labels.destination} code (optional)
          <input
            className="form-input uppercase"
            name={`${prefix}.destinationCode`}
      ...
      {booked && mode === "train" && (
        <details className="mt-4 rounded-xl border border-line/80 p-3">
          ...
          <div className="mt-3 grid gap-4 sm:grid-cols-2">
            <label className="form-label">
              Booked from station (optional)
              <input
                className="form-input"
                name={`${prefix}.bookedFromName`}
                placeholder="Only add this if it differs from the boarding station"
              />
            </label>
            <label className="form-label">
              Booked from code (optional)
              <input
                className="form-input uppercase"
                name={`${prefix}.bookedFromCode`}
                placeholder="Enter the station code printed under Booked From"
              />
            </label>
            <label className="form-label">
            ...
            <label className="form-label">
              ...
              <input
                className="form-input"
                name={`${prefix}.bookingStatus`}
                placeholder="Enter the original booking status"
              />
            </label>
            <label className="form-label">
              ...
              <input
                className="form-input"
                name={`${prefix}.currentStatus`}
                placeholder="Enter the latest confirmed or waitlist status"
              />
            </label>
          </div>
        </details>
      ...
      {booked && mode === "bus" && (
        <details className="mt-4 rounded-xl border border-line/80 p-3">
          ...
          <div className="mt-3 grid gap-4 sm:grid-cols-2">
            <label className="form-label">
              Bus class or layout (optional)
              <input
                className="form-input"
                name={`${prefix}.busClass`}
                placeholder="Enter the bus class or seating layout shown on the ticket"
              />
            </label>
            <label className="form-label">
              Boarding point details (optional)
              <textarea
                className="form-input min-h-20"
                name={`${prefix}.boardingPointDetails`}
                placeholder="Add landmark or pickup instructions from the ticket"
              />
            </label>
            <label className="form-label">
              Drop-off point details (optional)
              <textarea
                className="form-input min-h-20"
                name={`${prefix}.dropoffPointDetails`}
...
export function AddConnectionButton({
  ...
  return (
    <button type="button" className="secondary-button w-full" onClick={onClick}>
      <Plus className="size-4" /> Add connecting {mode === "ferry" ? "sailing" : mode}
    </button>
  );
}
...
export function CabFields({
  ...
  const booked = reservationState === "booked";
  const completed = reservationState === "walk_up";
  return (
    <>
      <fieldset className="rounded-2xl border border-line p-4">
        <legend className="px-1 text-sm font-extrabold">Ride type</legend>
        <div className="mt-1 grid gap-2 sm:grid-cols-2">
          {(
            ...
            <label
              key={value}
              className={`flex items-center gap-2 rounded-xl border p-3 text-sm font-bold ${rideType === value ? "border-brand bg-brand-soft" : "border-line bg-elevated"}`}
            >
              <input
                type="radio"
                name="cabRideType"
        ...
        <div className="mt-2 grid gap-4 sm:grid-cols-2">
          <label className="form-label">
            ...
            <input
              className="form-input"
              name="cab.pickup"
              placeholder="Enter where the cab should collect you"
              required
            />
          </label>
          <label className="form-label">
            Drop-off {rideType === "hourly" ? "(optional)" : <RequiredMark />}
            <input
              className="form-input"
              name="cab.dropoff"
              placeholder={
                rideType === "hourly"
                  ? "Enter the final drop-off if already known"
                  : "Enter where the ride should end"
              }
              required={rideType !== "hourly"}
            />
          </label>
        </div>
      </fieldset>
      ...
      {(booked || completed) && (
        <details open className="rounded-2xl border border-line p-4">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <div className="form-label">
              <span>Cab company or app {completed ? "(optional)" : <RequiredMark />}</span>
              <JourneyOperatorPicker mode="cab" name="cab.operator" required={booked} />
            </div>
            <label className="form-label">
              Vehicle / service class (optional)
              <input
                className="form-input"
                name="cab.vehicleClass"
      ...
      <details className="rounded-2xl border border-line p-4">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <label className="form-label">
            Pickup Google Maps link (optional)
            <input
              className="form-input"
              type="url"
              name="mapUrl"
              placeholder="Paste the pickup place or directions link"
            />
          </label>
          <label className="form-label">
            ...
            <input
              className="form-input"
              name="cab.driverName"
              placeholder="Add it later when a driver is assigned"
            />
          </label>
          <label className="form-label">
          ...
          <label className="form-label">
            ...
            <input
              className="form-input uppercase"
              name="cab.vehicleRegistration"
              placeholder="Add it later when the vehicle is assigned"
            />
          </label>
          {rideType === "airport_transfer" && (
            <>
              <label className="form-label">
                ...
                <select
                  className="form-input"
                  name="cab.linkedFlightLegId"
                  defaultValue=""
                  disabled={!flightLegs.length}
                >
                  <option value="">
                    {flightLegs.length
                      ? "Choose a flight in this trip"
                      : "No flights have been added to this trip"}
                  </option>
                  {flightLegs.map((flight) => (
                    <option
                      key={flight.id}
                      value={flight.id}
                    >{`${flight.airline_name} ${flight.flight_number} · ${flight.departure_airport_code || flight.departure_airport_name} → ${flight.arrival_airport_code || flight.arrival_airport_name}`}</option>
                  ))}
                </select>
              </label>
              <label className="form-label">
                Pickup buffer after landing (minutes)
                <input
                  className="form-input"
                  type="number"
                  min="0"
                  name="cab.pickupBuffer"
                  placeholder="Enter how long after landing pickup is planned"
                />
              </label>
              <label className="form-label">
                Luggage count (optional)
                <input
                  className="form-input"
                  type="number"
                  min="0"
                  name="cab.luggageCount"
          ...
          <label className="form-label sm:col-span-2">
            ...
            <textarea
              className="form-input min-h-20"
              name="notes"
              placeholder="Add instructions you may need for this ride"
            />
          </label>
        </div>
      </details>
