...
export function EventTitleField({ type }: { type: TimelineEventType }) {
  ...
  return <label className="form-label">{labels[type] ?? "Timeline title"}<RequiredMark /><input autoFocus className="form-input" name="title" placeholder="Describe what should appear on the timeline" required /></label>;
}
...
export function BookingFields({ type, referenceRequired = false, hideProvider = false }: { type: BookableEventType; referenceRequired?: boolean; hideProvider?: boolean }) {
  ...
  return <fieldset className="rounded-2xl border border-line p-4"><legend className="px-1 text-sm font-extrabold">Booking details</legend><div className="mt-2 grid gap-4 sm:grid-cols-2">
    {!hideProvider && <label className="form-label">{labels.provider}{type === "hotel_check_in" && <RequiredMark />}<input className="form-input" name="provider" placeholder={`Enter the ${labels.provider.toLocaleLowerCase()} shown on the confirmation`} required={type === "hotel_check_in"} /></label>}
    <label className="form-label">{labels.reference}{referenceRequired ? <RequiredMark /> : " (optional)"}<input className="form-input uppercase" name="referenceCode" maxLength={160} placeholder="Enter the reference shown on the confirmation" required={referenceRequired} /></label>
...
export function HotelStayFields({ trip }: { trip: Trip }) {
  ...
  return <fieldset className="rounded-2xl border border-line p-4"><legend className="px-1 text-sm font-extrabold">Stay</legend><div className="mt-2 grid gap-4 sm:grid-cols-2">
    <label className="form-label">Check-in date<RequiredMark /><input className="form-input" name="checkInDate" type="date" min={trip.start_date} max={trip.end_date} value={checkInDate} onChange={(event) => { const next = event.target.value; setCheckInDate(next); if (checkoutDate < next) setCheckoutDate(next); }} required /></label>
    ...
    <label className="form-label">Checkout date<RequiredMark /><input className="form-input" name="checkoutDate" type="date" min={checkInDate || trip.start_date} max={trip.end_date} value={checkoutDate} onChange={(event) => setCheckoutDate(event.target.value)} required /></label>
...
export function OtherTransportFields({ subtype, onSubtypeChange, trip }: { subtype: OtherTransportSubtype; onSubtypeChange: (value: OtherTransportSubtype) => void; trip: Trip }) {
  ...
  return <>
    ...
    <fieldset className="rounded-2xl border border-line p-4"><legend className="px-1 text-sm font-extrabold">{rental ? "Rental route" : "Route"}</legend><div className="mt-2 grid gap-4 sm:grid-cols-2"><label className="form-label">{rental ? "Pickup place" : "From"}<RequiredMark /><input className="form-input" name="transport.from" value={from} onChange={(event) => setFrom(event.target.value)} placeholder={rental ? "Enter where the vehicle will be collected" : "Enter where this journey starts"} required /></label><label className="form-label">{rental ? "Return place" : "To"}<RequiredMark /><input className="form-input" name="transport.to" value={to} onChange={(event) => setTo(event.target.value)} placeholder={rental ? "Enter where the vehicle will be returned" : "Enter where this journey ends"} required /></label>{rental && <label className="form-label sm:col-span-2">Return date and time (optional)<input className="form-input" type="datetime-local" name="transport.returnAt" min={`${trip.start_date}T00:00`} max={`${trip.end_date}T23:59`} /></label>}</div></fieldset>
...
export function CostFields({ trip, travelers }: { trip: Trip; travelers: Traveler[] }) {
  ...
  return <details className="rounded-2xl border border-line p-4"><summary className="cursor-pointer text-sm font-extrabold">Cost</summary><div className="mt-4 space-y-4">
    <p className="text-xs leading-5 text-muted">Optional. Enter an amount to add this cost now, or leave it blank and add the cost from the event later.</p>
    <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_minmax(9rem,12rem)]"><label className="form-label">Amount (optional)<input className="form-input" name="costAmount" type="number" inputMode="decimal" min="0.01" step="0.01" placeholder="Enter the total amount for this event" /></label><label className="form-label">Currency<CurrencySelect name="costCurrency" defaultValue={trip.base_currency} /></label></div>
    <details className="rounded-xl border border-line p-3"><summary className="cursor-pointer text-sm font-extrabold">Payment and sharing (optional)</summary><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="form-label">Cost label (optional)<input className="form-input" name="costTitle" placeholder="Describe what this amount covers" /></label><label className="form-label">Payment<select className="form-input" name="paymentStatus"><option value="planned">Planned / unpaid</option><option value="paid">Paid</option></select></label>{travelers.length > 0 && <label className="form-label">Paid by<select className="form-input" name="paidByTravelerId"><option value="">Not recorded yet</option>{travelers.map((traveler) => <option key={traveler.id} value={traveler.id}>{traveler.display_name}</option>)}</select></label>}{trip.expense_splitting_enabled && travelers.length > 0 && <div className="sm:col-span-2"><div className="flex flex-wrap gap-4 text-sm"><label className="flex items-center gap-2"><input type="radio" checked={everyone} onChange={() => setEveryone(true)} /> Split among everyone</label><label className="flex items-center gap-2"><input type="radio" checked={!everyone} onChange={() => setEveryone(false)} /> Choose people</label></div>{everyone ? travelers.map((traveler) => <input key={traveler.id} type="hidden" name="costTravelerIds" value={traveler.id} />) : <div className="mt-3 grid gap-2 sm:grid-cols-2">{travelers.map((traveler) => <label key={traveler.id} className="flex items-center gap-2 rounded-xl bg-elevated p-3 text-sm font-bold"><input type="checkbox" name="costTravelerIds" value={traveler.id} />{traveler.display_name}</label>)}</div>}</div>}</div></details>
  </div></details>;
}
