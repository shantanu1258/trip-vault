...
const bookingLabels: Partial<
  Record<BookableEventType, { provider: string; reference: string; phone: string }>
> = {
  flight: {
    provider: "Airline",
    reference: "Booking reference / PNR",
    phone: "Airline support phone"
  },
  hotel_check_in: {
    provider: "Hotel / property",
    reference: "Booking reference",
    phone: "Property phone"
  },
  activity: {
    provider: "Activity provider or venue",
    reference: "Booking reference",
    phone: "Provider phone"
  },
  meal: {
    provider: "Restaurant or venue",
    reference: "Reservation name or reference",
    phone: "Restaurant phone"
  },
  train: {
    provider: "Train operator",
    reference: "PNR or booking reference",
    phone: "Support phone"
  },
  bus: {
    provider: "Bus operator",
    reference: "Ticket / order number",
  ...
  cab: {
    provider: "Cab company or app",
    reference: "Booking reference / Ride ID",
    phone: "Support phone"
  },
  transport: {
...
export function EventTitleField({ type }: { type: TimelineEventType }) {
  ...
  return (
    <label className="form-label">
      {labels[type] ?? "Timeline title"}
      <RequiredMark />
      <input
        autoFocus
        className="form-input"
        name="title"
        placeholder="Describe what should appear on the timeline"
        required
      />
    </label>
  );
    ...
    return [
      {
        ...
        hint: "Keep the meal on the plan without a reservation."
      },
      {
        value: "booked",
        label: "Reserved",
        hint: "Add the restaurant confirmation and contact details."
      }
    ];
  if (type === "cab")
    return [
      { value: "planned", label: "Need a cab", hint: "Book or hail it when needed." },
      {
        ...
        hint: "A company or app has confirmed the ride."
      },
      {
        value: "walk_up",
        label: "Already took this ride",
        hint: "Record the actual route, cost, or receipt."
      }
    ];
  ...
  return [
    {
      value: "planned",
      label: "Plan only",
      hint: "Keep it on the timeline and add booking details later."
    },
    {
      value: "booked",
      label: type === "activity" ? "Booked" : "Reservation confirmed",
      hint: "Add the confirmation details now."
    }
  ];
}

export function ReservationStateFields({
  type,
  value,
  onChange,
  transportSubtype
  ...
  onChange: (value: ReservationState) => void;
  transportSubtype?: OtherTransportSubtype;
}) {
  ...
  if (
    ...
    return (
      <input
        ...
        value={type === "flight" ? "booked" : "planned"}
      />
    );
  return (
    <fieldset className="rounded-2xl border border-line p-4">
      <legend className="px-1 text-sm font-extrabold">Booking status</legend>
      <div
        className={`mt-1 grid gap-3 ${choices.length === 3 ? "sm:grid-cols-3" : "sm:grid-cols-2"}`}
      >
        {choices.map((choice) => (
          <label
            key={choice.value}
            className={`rounded-xl border p-3 text-sm transition ${value === choice.value ? "border-brand bg-brand-soft" : "border-line bg-elevated"}`}
          >
            ...
            <span className="mt-1 block text-xs leading-5 text-muted">{choice.hint}</span>
          </label>
        ))}
      </div>
    </fieldset>
    ...
    <FormSection open className="rounded-2xl border border-line p-4">
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        {!hideProvider && (
          <label className="form-label">
            {labels.provider}
            {type === "hotel_check_in" && <RequiredMark />}
            <input
              className="form-input"
              name="provider"
              placeholder={`Enter the ${labels.provider.toLocaleLowerCase()} shown on the confirmation`}
              required={type === "hotel_check_in"}
            />
          </label>
        )}
        ...
        <label className="form-label">
          ...
          <input
            ...
            placeholder="Paste the page used to view or manage this booking"
          />
        </label>
        {showContactName && (
          <label className="form-label">
            ...
            <input
              className="form-input"
              name="contactName"
              placeholder="Enter the person to contact, if one was provided"
            />
          </label>
        )}
        <label className="form-label">
          ...
          <input
            ...
            name="contactPhone"
            maxLength={40}
            placeholder="Include the country code for Call and WhatsApp"
          />
        </label>
      </div>
    </FormSection>
...
export function HotelStayFields({
  ...
  const [checkoutDate, setCheckoutDate] = useState(
    defaultHotelCheckoutLocal(initialCheckIn).slice(0, 10)
  );
    ...
    <>
      <label className="form-label">
        ...
        <input
          ...
          placeholder="Enter the number of diners"
        />
      </label>
      <label className="form-label">
        Dietary or arrival notes (optional)
        <textarea
          className="form-input min-h-20"
          name="dietaryNotes"
          placeholder="Add dietary needs or instructions for arrival"
        />
      </label>
    </>
  );
...
export function PlaceAndNotesFields({
  type,
  mealPartySize
}: {
  type: TimelineEventType;
        ...
        <div className="mt-4 grid gap-4">
          <label className="form-label">
            ...
            <input
              className="form-input"
              name="location"
              placeholder="Enter where this task needs to happen"
            />
          </label>
          <label className="form-label">
            Navigation (Google Maps link, optional)
            <input
              className="form-input"
              type="url"
              name="mapUrl"
              placeholder="Paste the place or directions link"
            />
          </label>
          <label className="form-label">
            Provider or organization (optional)
            <input
              className="form-input"
              name="provider"
              placeholder="Enter who provides or manages this task"
            />
          </label>
          <label className="form-label">
            External booking or manage link (optional)
            <input
              className="form-input"
              type="url"
              name="bookedViaUrl"
              placeholder="Paste the page used to book or manage this task"
            />
          </label>
          <label className="form-label">
            Notes (optional)
            <textarea
              ...
              name="notes"
              placeholder="Add instructions you may need before the trip"
            />
          </label>
        </div>
      </FormSection>
      ...
      <div className="mt-4 grid gap-4">
        ...
        <label className="form-label">
          ...
          <textarea
            className="form-input min-h-24"
            name="notes"
            placeholder="Add instructions you may need during the trip"
          />
        </label>
        {type === "hotel_check_in" && (
          <>
            <label className="form-label">
              ...
              <input
                className="form-input"
                name="roomType"
                placeholder="Enter the room type shown on the confirmation"
              />
            </label>
            <label className="form-label">
              ...
              <input
                ...
                min="1"
                inputMode="numeric"
                placeholder="Enter how many rooms are reserved"
              />
            </label>
            <label className="form-label">
              Lead guest (optional)
              <input
                className="form-input"
                name="leadGuest"
                placeholder="Enter the lead guest shown on the booking"
              />
            </label>
          </>
        )}
        {type === "activity" && (
          <label className="form-label">
            Entry or meeting instructions (optional)
            <textarea
              className="form-input min-h-20"
              name="meetingInstructions"
              placeholder="Enter the meeting point, entry rule, or arrival instruction"
            />
          </label>
        )}
...
export function OtherTransportFields({
  subtype,
  onSubtypeChange,
  trip
}: {
  subtype: OtherTransportSubtype;
  onSubtypeChange: (value: OtherTransportSubtype) => void;
  trip: Trip;
}) {
  ...
  return (
    <>
      <fieldset className="rounded-2xl border border-line p-4">
        <legend className="px-1 text-sm font-extrabold">Kind of transport</legend>
        <label className="form-label mt-2">
          ...
          <select
            ...
            value={subtype}
            onChange={(event) => onSubtypeChange(event.target.value as OtherTransportSubtype)}
          >
      ...
      <fieldset className="rounded-2xl border border-line p-4">
        <legend className="px-1 text-sm font-extrabold">{rental ? "Rental route" : "Route"}</legend>
        <div className="mt-2 grid gap-4 sm:grid-cols-2">
          <label className="form-label">
            {rental ? "Pickup place" : "From"}
            <RequiredMark />
            <input
              className="form-input"
              name="transport.from"
              ...
              placeholder={
                rental
                  ? "Enter where the vehicle will be collected"
                  : "Enter where this journey starts"
              }
              required
            />
          </label>
          <label className="form-label">
            {rental ? "Return place" : "To"}
            <RequiredMark />
            <input
              className="form-input"
              name="transport.to"
              ...
              placeholder={
                rental
                  ? "Enter where the vehicle will be returned"
                  : "Enter where this journey ends"
              }
              required
            />
          </label>
        ...
        <div className="mt-4 grid gap-4">
          <label className="form-label">
            ...
            <input
              ...
              type="url"
              name="mapUrl"
              placeholder="Paste the route, pickup, or destination link"
            />
          </label>
          <label className="form-label">
            Notes (optional)
            <textarea
              className="form-input min-h-24"
              name="notes"
              placeholder="Add tickets, pickup, return, or route instructions you may need"
            />
          </label>
        </div>
      </FormSection>
      ...
      <div className="mt-4 space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_minmax(9rem,12rem)]">
          <label className="form-label">
            ...
            <input
              ...
              min="0.01"
              step="0.01"
              placeholder="Enter the total amount for this event"
            />
          </label>
          <label className="form-label">
            Currency
        ...
        <FormSection className="rounded-xl border border-line p-3">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            ...
            <label className="form-label">
              Payment
              <select className="form-input" name="paymentStatus">
            ...
            {travelers.length > 0 && (
              <label className="form-label">
                Paid by
                <select className="form-input" name="paidByTravelerId">
            ...
            {trip.expense_splitting_enabled && travelers.length > 0 && (
                ...
                {!everyone && (
                  <div className="mt-3 grid gap-2 sm:grid-cols-2">
                    {travelers.map((traveler) => (
                      ...
                      >
                        <input type="checkbox" name="costTravelerIds" value={traveler.id} />
                        {traveler.display_name}
                      </label>
                    ))}
                  </div>
