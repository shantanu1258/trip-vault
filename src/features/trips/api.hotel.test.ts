...
describe("hotel timeline edit safety", () => {
  ...
  it.each(["hotel_check_in", "hotel_check_out"] as const)(
    "rejects a generic edit of a linked %s milestone before any network write",
    async (eventType) => {
      const existing: ItineraryItem = {
        ...
        title:
          eventType === "hotel_check_in" ? "Check in · Palm Springs" : "Check out · Palm Springs",
        event_type: eventType,
        starts_at:
          eventType === "hotel_check_in" ? "2026-09-28T08:00:00.000Z" : "2026-09-30T04:00:00.000Z",
        ends_at: null,
        timezone: "Asia/Dubai",
