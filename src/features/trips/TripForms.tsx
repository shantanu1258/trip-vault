  ...
  return (
    <ModalSheet eyebrow={trip.title} title={item ? "Edit itinerary item" : "Add itinerary item"} onClose={onClose}>
      <form ref={draft.formRef} className="mt-6 space-y-4" onSubmit={submit}>
        ...
        <label className="form-label">Title<input className="form-input" name="title" placeholder="Name this item so it is easy to find on the timeline" defaultValue={item?.title} autoFocus /></label>
...
export function AddCostForm({ trip, travelers, cost, bookingId, itineraryItemId, sourceTitle, onClose }: { trip: Trip; travelers: Traveler[]; cost?: TripCost; bookingId?: string; itineraryItemId?: string; sourceTitle?: string; onClose: () => void }) {
  ...
  return (
    <ModalSheet eyebrow={trip.title} title={cost ? "Edit trip cost" : "Add a trip cost"} onClose={onClose}>
      <form ref={draft.formRef} className="mt-6 space-y-4" onSubmit={submit}>
        <label className="form-label">What was it for?<input className="form-input" name="title" placeholder="Name the expense so travelers can recognize it" defaultValue={cost?.title ?? sourceTitle} autoFocus /><span className="mt-1 block text-xs font-normal text-muted">{sourceTitle && !cost ? "This cost stays attached to the timeline event; you may adjust its label." : "Use a short name such as Airport cab or Museum tickets."}</span></label>
...
export function TripSettingsForm({ trip, onClose, onArchived }: { trip: Trip; onClose: () => void; onArchived: () => void }) {
  ...
  const destroy = useMutation({ mutationFn: () => deleteTripPermanently(trip), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ["trips"] }); onArchived(); } });
  const submit = (event: FormEvent<HTMLFormElement>) => {
