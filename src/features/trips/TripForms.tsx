...
export function AddCostForm({ trip, travelers, cost, bookingId, itineraryItemId, sourceTitle, onClose }: { trip: Trip; travelers: Traveler[]; cost?: TripCost; bookingId?: string; itineraryItemId?: string; sourceTitle?: string; onClose: () => void }) {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!parsed.success) { setMessage(firstValidationMessage(parsed.error)); return; }
    const participantTravelerIds = form.getAll("travelerIds").map(String);
  ...
  return (
    <ModalSheet eyebrow={trip.title} title={cost ? "Edit trip cost" : "Add a trip cost"} onClose={onClose}>
      <form ref={draft.formRef} className="mt-6 space-y-4" onSubmit={submit}>
        ...
        {travelers.length > 0 && <><label className="form-label">Paid by<select className="form-input" name="paidByTravelerId" defaultValue={cost?.paid_by_traveler_id ?? ""}><option value="">Not recorded yet</option>{travelers.map((traveler) => <option key={traveler.id} value={traveler.id}>{traveler.display_name}</option>)}</select></label><ParticipantSelector travelers={travelers} explicitAll selectedTravelerIds={cost?.participants?.length ? cost.participants.map((participant) => participant.traveler_id) : undefined} /><p className="-mt-2 text-xs leading-5 text-muted">For now, the amount is split equally among the selected travelers. Balances are calculated separately for each currency.</p></>}
