...
import { addItineraryItem, addTripCost, archiveTrip, deleteTripPermanently, deleteTripRecoverably, updateItineraryItem, updateTrip, updateTripCost } from "./api";
import { getErrorMessage } from "./presentation";
import { timelineEventTypes, type CreateCostInput, type CreateItineraryInput, type ItineraryItem, type Trip, type TripCost } from "./types";
import { amountStringToMinor, costFormSchema, currencyFractionDigits, firstValidationMessage, itineraryFormSchema, tripFormSchema } from "./validation";
...
export function AddItineraryForm({ trip, travelers, bookings = [], item, preferredTravelerId, onAddBooking, onClose }: { trip: Trip; travelers: Traveler[]; bookings?: Booking[]; item?: ItineraryItem; preferredTravelerId?: string; onAddBooking?: () => void; onClose: () => void }) {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const parsed = itineraryFormSchema.safeParse({ title: form.get("title"), startsAt: timing.startsAt, endsAt: timing.endsAt || undefined, location: form.get("location") || undefined, notes: form.get("notes") || undefined });
  ...
  return (
    <ModalSheet eyebrow={trip.title} title={item ? "Edit itinerary item" : "Add itinerary item"} onClose={onClose}>
      <form ref={draft.formRef} className="mt-6 space-y-4" onSubmit={submit}>
        <label className="form-label">Event type<RequiredMark /><select className="form-input capitalize" name="eventType" defaultValue={item?.event_type ?? "activity"}>{timelineEventTypes.filter((type) => !["flight", "train", "bus", "ferry", "cab", "hotel_check_in", "hotel_check_out"].includes(type) || type === item?.event_type).map((type) => <option key={type} value={type}>{type.replaceAll("_", " ")}</option>)}</select></label>
        <label className="form-label">Title<RequiredMark /><input className="form-input" name="title" placeholder="Name this item so it is easy to find on the timeline" defaultValue={item?.title} autoFocus /></label>
...
export function AddCostForm({ trip, travelers, cost, bookingId, itineraryItemId, sourceTitle, onClose }: { trip: Trip; travelers: Traveler[]; cost?: TripCost; bookingId?: string; itineraryItemId?: string; sourceTitle?: string; onClose: () => void }) {
  const queryClient = useQueryClient();
  const [message, setMessage] = useState("");
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const participantTravelerIds = trip.expense_splitting_enabled
      ...
      : cost?.participants?.length ? cost.participants.map((participant) => participant.traveler_id) : travelers.map((traveler) => traveler.id);
    if (travelers.length > 0 && participantTravelerIds.length === 0) { setMessage("Choose at least one traveler to share this cost."); return; }
    mutation.mutate({
      tripId: trip.id,
  ...
  return (
    <ModalSheet eyebrow={trip.title} title={cost ? "Edit trip cost" : "Add a trip cost"} onClose={onClose}>
      <form ref={draft.formRef} className="mt-6 space-y-4" onSubmit={submit}>
        <label className="form-label">What was it for?<RequiredMark /><input className="form-input" name="title" placeholder="Name the expense so travelers can recognize it" defaultValue={cost?.title ?? sourceTitle} autoFocus /><span className="mt-1 block text-xs font-normal text-muted">{sourceTitle && !cost ? "This cost stays attached to the timeline event; you may adjust its label." : "Use a short name such as Airport cab or Museum tickets."}</span></label>
        ...
        <div className="grid grid-cols-[minmax(0,1fr)_minmax(9rem,12rem)] gap-4"><label className="form-label">Amount<RequiredMark /><input className="form-input" name="amount" inputMode="decimal" placeholder="Enter 0 if this item was free" defaultValue={cost ? (cost.amount_minor / 10 ** currencyFractionDigits(cost.currency_code)).toFixed(currencyFractionDigits(cost.currency_code)) : ""} /></label><label className="form-label">Currency<RequiredMark /><CurrencySelect name="currencyCode" defaultValue={cost?.currency_code ?? trip.base_currency} /></label></div>
        {travelers.length > 0 && <><label className="form-label">Paid by<select className="form-input" name="paidByTravelerId" defaultValue={cost?.paid_by_traveler_id ?? ""}><option value="">Not recorded yet</option>{travelers.map((traveler) => <option key={traveler.id} value={traveler.id}>{traveler.display_name}</option>)}</select></label>{trip.expense_splitting_enabled && <><ParticipantSelector travelers={travelers} explicitAll selectedTravelerIds={cost?.participants?.length ? cost.participants.map((participant) => participant.traveler_id) : undefined} /><p className="-mt-2 text-xs leading-5 text-muted">The amount is split equally among the selected travelers. Balances are calculated separately for each currency.</p></>}</>}
...
export function TripSettingsForm({ trip, onClose, onArchived }: { trip: Trip; onClose: () => void; onArchived: () => void }) {
  ...
  const destroy = useMutation({
    ...
    onSuccess: async () => {
      const belongsToDeletedTrip = ({ queryKey }: { queryKey: readonly unknown[] }) => queryKey.includes(trip.id);
  ...
  return <ModalSheet eyebrow={trip.title} title="Trip settings" onClose={onClose}><form className="mt-6 space-y-4" onSubmit={submit}><label className="form-label">Trip name<input className="form-input" name="title" defaultValue={trip.title} /></label><label className="form-label">Destination<input className="form-input" name="destination" defaultValue={trip.destination_summary} /></label><div className="grid gap-4 sm:grid-cols-2"><label className="form-label">Start date<input className="form-input" name="startDate" type="date" defaultValue={trip.start_date} /></label><label className="form-label">End date<input className="form-input" name="endDate" type="date" defaultValue={trip.end_date} /></label></div><input type="hidden" name="timezone" value={trip.primary_timezone} /><label className="form-label sm:max-w-64">Currency<CurrencySelect name="baseCurrency" defaultValue={trip.base_currency} /></label><p className="text-xs leading-5 text-muted">Journey time zones live on each departure and arrival, so changing general trip details cannot shift ticket times.</p>{(message || update.error || archive.error || remove.error || destroy.error) && <p role="alert" className="rounded-xl bg-danger/10 p-3 text-sm font-bold text-danger">{message || getErrorMessage(update.error || archive.error || remove.error || destroy.error)}</p>}<button className="primary-button w-full" disabled={update.isPending}><Save className="size-4" /> Save trip</button><div className="border-t border-line pt-4"><p className="text-xs leading-5 text-muted">Archiving removes the trip from Home and keeps its cloud and local records recoverable.</p><button type="button" className="secondary-button mt-3 text-danger" disabled={archive.isPending} onClick={async () => { if (await confirm({ title: "Archive trip?", message: `Archive ${trip.title}?`, confirmLabel: "Archive", tone: "danger" })) archive.mutate(); }}><Archive className="size-4" /> Archive trip</button><p className="mt-5 text-xs leading-5 text-muted">Recently deleted trips can be restored for 30 days. This action requires a connection.</p><button type="button" className="secondary-button mt-3 text-danger" disabled={remove.isPending || !navigator.onLine} onClick={async () => { if (await confirm({ title: "Move trip to Recently deleted?", message: `Move ${trip.title} to Recently deleted?`, confirmLabel: "Move trip", tone: "danger" })) remove.mutate(); }}><Trash2 className="size-4" /> Move to Recently deleted</button><div className="mt-6 rounded-2xl border border-danger/30 bg-danger/5 p-4"><p className="font-extrabold text-danger">Temporary testing tool</p><p className="mt-1 text-xs leading-5 text-muted">Permanently removes this trip, its events, costs, and cloud documents. This cannot be restored. Type the exact trip name to enable it.</p><input className="form-input mt-3" value={deleteConfirmation} onChange={(event) => setDeleteConfirmation(event.target.value)} placeholder={`Type ${trip.title} to confirm`} aria-label="Confirm permanent trip deletion" /><button type="button" className="secondary-button mt-3 w-full text-danger" disabled={!navigator.onLine || destroy.isPending || deleteConfirmation !== trip.title} onClick={() => destroy.mutate()}><Trash2 className="size-4" /> Permanently delete test trip</button></div></div></form></ModalSheet>;
}
