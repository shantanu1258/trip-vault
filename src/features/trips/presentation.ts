...
export function tripPhase(trip: Trip, now = new Date()): TripPhase {
  ...
  if (currentDate >= previousCalendarDay(trip.start_date) && currentDate <= trip.end_date)
    return "current";
...
export function selectFocusedTrip(trips: Trip[], savedTripId?: string | null, now = new Date()) {
  ...
  if (active.length) {
    const saved = active.find((trip) => trip.id === savedTripId);
...
export function currentItineraryItem(items: ItineraryItem[], now = new Date()) {
  ...
  return (
    items.find(
      (item) =>
        !item.is_all_day &&
        new Date(item.starts_at).getTime() <= time &&
        new Date(item.ends_at ?? item.starts_at).getTime() >= time
    ) ?? items.find((item) => new Date(item.starts_at).getTime() >= time)
  );
}

export function moveEqualTimeItem(
  items: ItineraryItem[],
  itemId: string,
  direction: "up" | "down"
) {
  ...
  if (
    ...
    items[currentIndex].starts_at !== items[targetIndex].starts_at
  )
    return items;
  ...
  return reordered.map((item, index) => ({
    ...item,
    sort_key: `${String(index).padStart(8, "0")}:${item.id}`
  }));
}
