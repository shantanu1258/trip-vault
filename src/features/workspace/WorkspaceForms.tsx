...
export function UploadDocumentForm({ trip, travelers, preferredTravelerId, onClose, onUploaded, bookingId, flightLegId, members = [], privateOnly = false }: { trip: Trip; travelers: Traveler[]; preferredTravelerId?: string; onClose: () => void; onUploaded?: (documentId: string) => void | Promise<void>; bookingId?: string; flightLegId?: string; members?: TripMember[]; privateOnly?: boolean }) {
  ...
  const queryClient = useQueryClient(); const [kind, setKind] = useState<DocumentKind>(initialKind); const [assignmentMode, setAssignmentMode] = useState<DocumentAssignmentMode>(documentKind(initialKind).defaultAssignment); const [selectedTravelerIds, setSelectedTravelerIds] = useState<string[]>(preferredTravelerId ? [preferredTravelerId] : []); const [visibility, setVisibility] = useState<DocumentVisibility>("private"); const [selectedFile, setSelectedFile] = useState<File | null>(null); const [message, setMessage] = useState(""); const [queuedMessage, setQueuedMessage] = useState("");
  const title = suggestedDocumentTitle(kind, assignmentMode, selectedTravelerIds, travelers);
  ...
  return <ModalSheet eyebrow={trip.title} title="Upload a document" onClose={onClose}><form ref={draft.formRef} className="mt-6 space-y-4" onSubmit={submit}>
    ...
    <div className="rounded-2xl bg-elevated p-4"><p className="text-xs font-bold text-muted">Saved Vault name</p><p className="mt-1 font-display text-lg font-black">{title}</p><p className="mt-1 text-xs text-muted">The original filename is kept separately. Change the document type or traveler to update this name.</p></div>
