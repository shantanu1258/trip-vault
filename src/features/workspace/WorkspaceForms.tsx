...
export function UploadDocumentForm({ trip, travelers, preferredTravelerId, onClose, onUploaded, bookingId, flightLegId, journeyLegId, contextTitle, members = [], privateOnly = false, initialFile = null, initialKind, assignmentPreset }: { trip: Trip; travelers: Traveler[]; preferredTravelerId?: string; onClose: () => void; onUploaded?: (documentId: string) => void | Promise<void>; bookingId?: string; flightLegId?: string; journeyLegId?: string; contextTitle?: string; members?: TripMember[]; privateOnly?: boolean; initialFile?: File | null; initialKind?: DocumentKind; assignmentPreset?: DocumentAssignmentPreset }) {
  ...
  const queryClient = useQueryClient(); const [kind, setKind] = useState<DocumentKind>(resolvedInitialKind); const [assignmentMode, setAssignmentMode] = useState<DocumentAssignmentMode>(assignmentPreset?.mode ?? documentKind(resolvedInitialKind).defaultAssignment); const [selectedTravelerIds, setSelectedTravelerIds] = useState<string[]>(assignmentPreset?.mode === "selected" ? assignmentPreset.travelerIds : preferredTravelerId ? [preferredTravelerId] : []); const [visibility, setVisibility] = useState<DocumentVisibility>(privateOnly ? "private" : "trip"); const [selectedFile, setSelectedFile] = useState<File | null>(initialFile); const [customTitle, setCustomTitle] = useState(""); const [message, setMessage] = useState(""); const [queuedMessage, setQueuedMessage] = useState("");
  const assignedTravelerNames = selectedTravelerIds.map((id) => travelers.find((traveler) => traveler.id === id)?.display_name).filter((name): name is string => Boolean(name));
