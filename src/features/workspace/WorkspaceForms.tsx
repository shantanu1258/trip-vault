...
export function UploadDocumentForm({ trip, travelers, preferredTravelerId, onClose, onUploaded, bookingId, flightLegId, journeyLegId, contextTitle, members = [], privateOnly = false, initialFile = null, initialKind }: { trip: Trip; travelers: Traveler[]; preferredTravelerId?: string; onClose: () => void; onUploaded?: (documentId: string) => void | Promise<void>; bookingId?: string; flightLegId?: string; journeyLegId?: string; contextTitle?: string; members?: TripMember[]; privateOnly?: boolean; initialFile?: File | null; initialKind?: DocumentKind }) {
  ...
  const queryClient = useQueryClient(); const [kind, setKind] = useState<DocumentKind>(resolvedInitialKind); const [assignmentMode, setAssignmentMode] = useState<DocumentAssignmentMode>(documentKind(resolvedInitialKind).defaultAssignment); const [selectedTravelerIds, setSelectedTravelerIds] = useState<string[]>(preferredTravelerId ? [preferredTravelerId] : []); const [visibility, setVisibility] = useState<DocumentVisibility>(privateOnly ? "private" : "trip"); const [selectedFile, setSelectedFile] = useState<File | null>(initialFile); const [customTitle, setCustomTitle] = useState(""); const [message, setMessage] = useState(""); const [queuedMessage, setQueuedMessage] = useState("");
