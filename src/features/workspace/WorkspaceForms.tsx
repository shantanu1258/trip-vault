...
import { CalendarPlus, Check, Clipboard, FileUp, Loader2, NotebookPen, RefreshCw, TicketCheck, Trash2, UserPlus, UsersRound } from "lucide-react";
...
export function ShareTripForm({ trip, travelers, onClose }: { trip: Trip; travelers: Traveler[]; onClose: () => void }) {
  ...
  const [message, setMessage] = useState(""); const [code, setCode] = useState(""); const [copied, setCopied] = useState(false); const [offerSent, setOfferSent] = useState("");
  const invitations = useQuery({ queryKey: ["invitations", trip.id], queryFn: () => listInvitations(trip.id), enabled: navigator.onLine });
  const associated = useQuery({ queryKey: ["associated-accounts"], queryFn: listAssociatedAccounts, enabled: navigator.onLine });
  ...
  const offer = useMutation({ mutationFn: createTripMembershipOffer, onSuccess: (_, input) => { const account = associated.data?.find((item) => item.user_id === input.userId); setOfferSent(`${account?.display_name ?? "This account"} can now accept ${trip.title} from Home.`); } });
  ...
  const submitCode = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setMessage(""); const form = new FormData(event.currentTarget); const travelerId = String(form.get("travelerId") ?? ""); if (targetType === "traveler" && !travelerId) { setMessage("Select the person this code belongs to."); return; } mutation.mutate({ tripId: trip.id, targetType, travelerId: targetType === "traveler" ? travelerId : undefined, role: String(form.get("role")) as "editor" | "viewer" }); };
  const submitKnown = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setMessage(""); setOfferSent(""); const form = new FormData(event.currentTarget); const userId = String(form.get("userId") ?? ""); const travelerId = String(form.get("knownTravelerId") ?? ""); if (!userId) { setMessage("Choose a previously associated account."); return; } if (targetType === "traveler" && !travelerId) { setMessage("Choose which traveler profile this account belongs to."); return; } offer.mutate({ tripId: trip.id, userId, targetType, travelerId: targetType === "traveler" ? travelerId : undefined, role: String(form.get("knownRole")) as "editor" | "viewer" }); };
  ...
  const TargetType = () => <div className="grid grid-cols-2 rounded-2xl bg-elevated p-1"><button type="button" onClick={() => setTargetType("traveler")} className={`tap-target rounded-xl text-sm font-bold ${targetType === "traveler" ? "bg-surface shadow-soft" : "text-muted"}`}>Traveler</button><button type="button" onClick={() => setTargetType("collaborator")} className={`tap-target rounded-xl text-sm font-bold ${targetType === "collaborator" ? "bg-surface shadow-soft" : "text-muted"}`}>Non-traveling helper</button></div>;

  return <ModalSheet eyebrow={trip.title} title="Share this trip" onClose={onClose}>
    {!code && <div className="mt-6 grid grid-cols-2 rounded-2xl border border-line p-1"><button type="button" onClick={() => { setMethod("code"); setMessage(""); }} className={`tap-target rounded-xl text-sm font-bold ${method === "code" ? "bg-brand text-surface" : "text-muted"}`}>Private code / QR</button><button type="button" onClick={() => { setMethod("known"); setMessage(""); }} className={`tap-target rounded-xl text-sm font-bold ${method === "known" ? "bg-brand text-surface" : "text-muted"}`}>Known account</button></div>}
    {code ? <div className="mt-7 text-center"><LocalQrCode value={joinUrl} /><p className="mt-5 text-sm text-muted">The recipient signs in, scans this QR, and confirms the prefilled one-time code. It expires after 14 days and cannot be reused.</p><button onClick={() => copy(code)} type="button" className="mt-5 inline-flex items-center gap-3 rounded-2xl bg-brand px-5 py-4 font-mono text-xl font-black tracking-[0.12em] text-surface">{code} {copied ? <Check className="size-5" /> : <Clipboard className="size-5" />}</button><button type="button" className="secondary-button mx-auto mt-3" onClick={() => copy(joinUrl)}><Clipboard className="size-4" /> Copy private link</button><button type="button" className="secondary-button mx-auto mt-3" onClick={() => { setCode(""); setCopied(false); }}>Create another code</button></div>
      : method === "known" ? <form onSubmit={submitKnown} className="mt-5 space-y-4"><TargetType /><label className="form-label">Previously associated account<select className="form-input" name="userId" defaultValue=""><option value="" disabled>Choose an account</option>{associated.data?.map((account) => <option key={account.user_id} value={account.user_id}>{account.display_name}</option>)}</select></label>{targetType === "traveler" && <label className="form-label">Which traveler are they?<select className="form-input" name="knownTravelerId" defaultValue=""><option value="" disabled>Select traveler</option>{travelers.map((traveler) => <option key={traveler.id} value={traveler.id}>{traveler.display_name}</option>)}</select></label>}<label className="form-label">App access<select className="form-input" name="knownRole" defaultValue="viewer"><option value="viewer">Viewer — use their relevant trip information</option><option value="editor">Editor — also edit the shared trip</option></select></label><p className="rounded-2xl bg-brand-soft p-4 text-sm leading-6 text-muted">No code is needed. The account receives a pending invitation on Home and must accept before the trip appears.</p>{associated.data?.length === 0 && <p className="rounded-xl border border-dashed border-line p-4 text-sm text-muted">No reusable accounts yet. After someone joins one of your trips with a code, they will appear here for future trips.</p>}{(message || offer.error) && <p role="alert" className="rounded-xl bg-danger/10 p-3 text-sm font-bold text-danger">{message || getErrorMessage(offer.error)}</p>}{offerSent && <p role="status" className="rounded-xl bg-success/10 p-3 text-sm font-bold text-success">{offerSent}</p>}<button className="primary-button w-full" disabled={offer.isPending || !navigator.onLine || !associated.data?.length}>{offer.isPending ? <Loader2 className="size-4 animate-spin" /> : <UsersRound className="size-4" />} Send invitation</button></form>
      : <form onSubmit={submitCode} className="mt-5 space-y-4"><TargetType />{targetType === "traveler" && <label className="form-label">Who is this code for?<select className="form-input" name="travelerId" defaultValue=""><option value="" disabled>Select traveler</option>{travelers.map((traveler) => <option key={traveler.id} value={traveler.id}>{traveler.display_name}</option>)}</select></label>}<label className="form-label">App access<select className="form-input" name="role" defaultValue="viewer"><option value="viewer">Viewer — can view and manage own documents</option><option value="editor">Editor — can also edit the shared trip</option></select></label><p className="rounded-2xl bg-brand-soft p-4 text-sm leading-6 text-muted">The recipient must sign in before the one-time code can reveal or join the trip.</p>{(message || mutation.error) && <p role="alert" className="rounded-xl bg-danger/10 p-3 text-sm font-bold text-danger">{message || getErrorMessage(mutation.error)}</p>}<button className="primary-button w-full" disabled={mutation.isPending || !navigator.onLine}>{mutation.isPending ? <Loader2 className="size-4 animate-spin" /> : <UsersRound className="size-4" />} Generate one-time code</button></form>}
    {method === "code" && <section className="mt-7 border-t border-line pt-5"><div className="flex items-center justify-between"><div><p className="eyebrow">Invitation history</p><p className="mt-1 text-sm text-muted">Codes themselves are intentionally never stored in readable form.</p></div>{invitations.isFetching && <Loader2 className="size-4 animate-spin text-muted" />}</div><div className="mt-3 space-y-2">{invitations.data?.slice(0, 10).map((invitation) => { const traveler = travelers.find((item) => item.id === invitation.traveler_id); const status = invitation.redeemed_at ? "used" : invitation.revoked_at ? "revoked" : new Date(invitation.expires_at) <= new Date() ? "expired" : "active"; return <div key={invitation.id} className="rounded-2xl border border-line bg-elevated p-3"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-bold">{invitation.target_type === "collaborator" ? "Non-traveling helper" : traveler?.display_name ?? "Traveler"} · {invitation.role}</p><p className="mt-1 text-xs capitalize text-muted">{status} · expires {new Date(invitation.expires_at).toLocaleDateString()}</p></div>{status === "active" && <div className="flex"><button type="button" disabled={replace.isPending} onClick={() => replace.mutate(invitation)} className="tap-target grid size-9 place-items-center text-brand" aria-label="Replace invitation code"><RefreshCw className="size-3.5" /></button><button type="button" disabled={revoke.isPending} onClick={() => revoke.mutate(invitation.id)} className="tap-target grid size-9 place-items-center text-danger" aria-label="Revoke invitation"><Trash2 className="size-3.5" /></button></div>}</div></div>; })}{invitations.data?.length === 0 && <p className="rounded-xl border border-dashed border-line p-4 text-sm text-muted">No invitations yet.</p>}{!navigator.onLine && <p className="text-sm text-warning">Invitation history and changes require a connection.</p>}</div></section>}
  </ModalSheet>;
}
...
export type DocumentAssignmentPreset = { mode: Extract<DocumentAssignmentMode, "shared" | "selected">; travelerIds: string[] };

export function UploadDocumentForm({ trip, travelers, preferredTravelerId, onClose, onUploaded, onCreateEvent, bookingId, flightLegId, journeyLegId, contextTitle, members = [], privateOnly = false, initialFile = null, initialKind, assignmentPreset }: { trip: Trip; travelers: Traveler[]; preferredTravelerId?: string; onClose: () => void; onUploaded?: (documentId: string) => void | Promise<void>; onCreateEvent?: (document: Pick<VaultDocument, "id" | "title">) => void | Promise<void>; bookingId?: string; flightLegId?: string; journeyLegId?: string; contextTitle?: string; members?: TripMember[]; privateOnly?: boolean; initialFile?: File | null; initialKind?: DocumentKind; assignmentPreset?: DocumentAssignmentPreset }) {
  ...
  const initialAssignment = assignmentPreset?.mode ?? (initialKind || flightLegId || journeyLegId || bookingId ? documentKind(resolvedInitialKind).defaultAssignment : "shared");
  const queryClient = useQueryClient(); const [kind, setKind] = useState<DocumentKind>(resolvedInitialKind); const [assignmentMode, setAssignmentMode] = useState<DocumentAssignmentMode>(initialAssignment); const [selectedTravelerIds, setSelectedTravelerIds] = useState<string[]>(assignmentPreset?.mode === "selected" ? assignmentPreset.travelerIds : preferredTravelerId ? [preferredTravelerId] : []); const [visibility, setVisibility] = useState<DocumentVisibility>(privateOnly ? "private" : "trip"); const [selectedFile, setSelectedFile] = useState<File | null>(initialFile); const [customTitle, setCustomTitle] = useState(""); const [message, setMessage] = useState(""); const [queuedMessage, setQueuedMessage] = useState(""); const [createEventAfterUpload, setCreateEventAfterUpload] = useState(false);
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (document) => {
      ...
      if (createEventAfterUpload && onCreateEvent) { await onCreateEvent({ id: document.id, title: document.title }); return; }
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!file) { setMessage("Choose a PDF or image."); return; }
    if (assignmentMode === "selected" && !travelerIds.length) { setMessage("Choose at least one traveler, or select Assign later."); return; }
    if (effectiveVisibility === "selected_members" && !selectedUserIds.length) { setMessage("Choose at least one signed-in member."); return; }
    const selectedKind = documentKind(kind);
    mutation.mutate({ tripId: trip.id, title, category: selectedKind.category, purpose: selectedKind.purpose, assignmentMode, visibility: effectiveVisibility, travelerIds, bookingId, flightLegId, journeyLegId, selectedUserIds, shortLabel: String(form.get("shortLabel") ?? "").trim() || undefined, file });
  };
  ...
  return <ModalSheet eyebrow={trip.title} title={initialFile ? "Finish attaching flight document" : "Upload a document"} onClose={onClose}><form ref={draft.formRef} className="mt-6 space-y-4" onSubmit={submit}>
    {initialFile && <p className="rounded-2xl bg-success/10 p-4 text-sm leading-6 text-muted"><strong className="block text-success">Flight saved safely</strong>The flight is already on the timeline. Confirm this file's type and visibility; its travelers are copied from the flight. If cloud upload fails, the attempted upload stays in your private document inbox on this device.</p>}
    ...
    <div className="rounded-2xl bg-elevated p-4"><p className="text-xs font-bold text-muted">Saved Vault name</p><p className="mt-1 font-display text-lg font-black">{title}</p>{customTitle.trim() && <p className="mt-2 text-xs text-muted">Trip context: {defaultTitle}</p>}<p className="mt-2 text-xs text-muted">Original device file: {selectedFile?.name ?? "Choose a file above"}</p></div>
    ...
    <label className="form-label">Short label (optional)<input className="form-input" name="shortLabel" placeholder="Add a seat, bag, or ticket detail shown on cards" disabled={Boolean(queuedMessage)} /></label>
    ...
    {onCreateEvent && <label className={`flex cursor-pointer items-start gap-3 rounded-2xl border p-4 ${createEventAfterUpload ? "border-brand bg-brand-soft" : "border-line bg-elevated"}`}><input type="checkbox" className="mt-1 size-4 accent-brand" checked={createEventAfterUpload} onChange={(event) => setCreateEventAfterUpload(event.target.checked)} /><span><strong className="flex items-center gap-2"><CalendarPlus className="size-4" /> Add a timeline event after upload</strong><span className="mt-1 block text-xs leading-5 text-muted">The file is saved first, then the event form opens and links this document automatically.</span></span></label>}
...
export function AddNoteForm({ trip, note, onClose }: { trip: Trip; note?: TripNote; onClose: () => void }) {
  ...
  return <ModalSheet eyebrow={trip.title} title={note ? "Edit trip note" : "Add a trip note"} onClose={onClose}><form ref={draft.formRef} onSubmit={submit} className="mt-6 space-y-4"><label className="form-label">Title (optional)<input className="form-input" name="title" placeholder="Name this note so it is easy to find" defaultValue={note?.title ?? ""} /></label><label className="form-label">Note<textarea autoFocus className="form-input min-h-40 resize-y" name="body" placeholder="Write the information you want available during the trip" defaultValue={note?.body ?? ""} /></label>{(message || mutation.error) && <p role="alert" className="text-sm font-bold text-danger">{message || getErrorMessage(mutation.error)}</p>}<button className="primary-button w-full"><NotebookPen className="size-4" /> {note ? "Save changes" : "Save note"}</button></form></ModalSheet>;
}
