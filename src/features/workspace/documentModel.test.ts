...
describe("document model", () => {
  ...
  it("uses travel-document defaults", () => {
    expect(documentKind("hotel_confirmation")).toMatchObject({
      category: "hotel",
      purpose: "hotel_confirmation",
  ...
  it("shows shared documents for every traveler but not unassigned tickets", () => {
    ...
    expect(
      documentMatchesTraveler(vaultDocument({ assignment_mode: "unassigned" }), "traveler-1")
    ).toBe(false);
    expect(
      documentMatchesTraveler(
        vaultDocument({ assignment_mode: "selected", traveler_ids: ["traveler-2"] }),
        "traveler-1"
      )
    ).toBe(false);
    expect(
      documentMatchesTraveler(
        vaultDocument({ assignment_mode: "selected", traveler_ids: ["traveler-1", "traveler-2"] }),
        "traveler-1"
      )
    ).toBe(true);
  });
  ...
  it("names a document from its type and intended traveler assignment", () => {
    expect(suggestedDocumentTitle("flight_ticket", "shared", [], travelers)).toBe(
      "Flight ticket · Everyone"
    );
    expect(suggestedDocumentTitle("boarding_pass", "selected", ["traveler-2"], travelers)).toBe(
      "Boarding pass · Ravi"
    );
    expect(suggestedDocumentTitle("visa", "selected", [], travelers)).toBe(
      "Visa · Choose traveler"
    );
    expect(suggestedDocumentTitle("activity_ticket", "unassigned", [], travelers)).toBe(
      "Activity admission ticket · Assign later"
    );
    expect(
      suggestedDocumentTitle(
        "boarding_pass",
    ...
    ).toBe("Boarding pass · Asha · Flight to Dubai");
  });
});
