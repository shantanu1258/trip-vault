...
export function readTravelerFocus(tripId: string) {
  try {
    return localStorage.getItem(travelerFocusKey(tripId));
  } catch {
    return null;
  }
}
...
export function itineraryMatchesTraveler(
  item: ItineraryItem,
  travelerId: string,
  ...
  return (
    ...
    participants.some((row) => row.itinerary_item_id === item.id && row.traveler_id === travelerId)
  );
}

export function requirementMatchesTraveler(
...
export function requirementAudienceLabel(
  requirementId: string,
  assignees: RequirementAssignee[],
...
export function filterTravelerWorkspace(input: {
  ...
  const itinerary = input.itinerary.filter((item) =>
    itineraryMatchesTraveler(item, input.travelerId!, input.participants)
  );
  const visibleItineraryIds = new Set(itinerary.map((item) => item.id));
  const bookingIds = new Set(
    input.bookingTravelers
      .filter((row) => row.traveler_id === input.travelerId)
  ...
  return {
    ...
    requirements: input.requirements.filter((requirement) =>
      requirementMatchesTraveler(requirement.id, input.travelerId!, input.requirementAssignees)
    ),
    documents: input.documents.filter((document) =>
      documentMatchesTraveler(document, input.travelerId!)
    )
  };
}
