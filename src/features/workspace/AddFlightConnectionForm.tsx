...
export function validateNextFlightConnection(
  lastLeg: FlightLeg,
  input: {
  ...
  const previousDestination = normalizedEndpoint(
    lastLeg.arrival_airport_code,
  ...
  if (
    input.journeyScope === "domestic" &&
    previousCountry &&
    input.arrivalCountryCode.toLocaleUpperCase() !== previousCountry
  ) {
    throw new Error(
      `A domestic connection must stay within ${previousCountry}. Choose International for another country.`
    );
  }
}

export function AddFlightConnectionForm({
  trip,
  booking,
  lastLeg,
  onClose
  ...
  const domesticCountry =
    journeyScope === "domestic"
      ? lastLeg.arrival_country_code?.trim().toLocaleUpperCase()
      : undefined;
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const departureTimezone = value(form, "departureTimezone");
      const arrivalTimezone = value(form, "arrivalTimezone");
      ...
      const boardingText = value(form, "boardingLeadMinutes");
      const boardingLeadMinutes = boardingText ? Number(boardingText) : undefined;
      if (
        boardingLeadMinutes !== undefined &&
      ...
      const airlineName = value(form, "airlineName");
      const departureName = value(form, "departureName");
      const arrivalName = value(form, "arrivalName");
      if (!airlineName || !departureName || !arrivalName)
        throw new Error("Choose the airline, departure airport, and arrival airport.");
      validateNextFlightConnection(lastLeg, {
        departureCode: value(form, "departureCode"),
        departureName,
        departureAt,
        arrivalAt,
        arrivalCountryCode: value(form, "arrivalCountry"),
        journeyScope
      });
      await Promise.allSettled([
        value(form, "airlineNameSource") === "other"
          ? suggestCatalogValue({ type: "airline", displayValue: airlineName })
          : Promise.resolve(),
        value(form, "departureNameSource") === "other"
          ? suggestCatalogValue({
              type: "airport",
              displayValue: departureName,
              proposedData: {
                code: value(form, "departureCode"),
                country_code: value(form, "departureCountry"),
                timezone: departureTimezone
              }
            })
          : Promise.resolve(),
        value(form, "arrivalNameSource") === "other"
          ? suggestCatalogValue({
              type: "airport",
              displayValue: arrivalName,
              proposedData: {
                code: value(form, "arrivalCode"),
                country_code: value(form, "arrivalCountry"),
      ...
      mutation.mutate({
        ...
        departureCode: value(form, "departureCode").toUpperCase(),
        departureName,
        departureCountryCode: value(form, "departureCountry").toUpperCase(),
        arrivalCode: value(form, "arrivalCode").toUpperCase(),
        arrivalName,
        arrivalCountryCode: value(form, "arrivalCountry").toUpperCase(),
        departureAt,
        arrivalAt,
        departureTimezone,
        arrivalTimezone,
  ...
  return (
    <ModalSheet eyebrow={booking.title} title="Add a connecting flight" onClose={onClose}>
      <p className="mt-3 rounded-2xl bg-brand-soft p-4 text-sm text-muted">
        The current journey ends at{" "}
        <strong className="text-ink">
          {lastLeg.arrival_airport_code || lastLeg.arrival_airport_name}
        </strong>{" "}
        on{" "}
        {new Intl.DateTimeFormat(undefined, {
          dateStyle: "medium",
          timeStyle: "short",
          timeZone: lastLeg.arrival_timezone
        }).format(new Date(lastLeg.scheduled_arrival_at))}
        . The new leg will become the next connection.
      </p>
      <form className="mt-5 space-y-5" onSubmit={submit}>
        ...
        <div className="rounded-2xl border border-line bg-elevated p-4">
          <p className="eyebrow">From airport</p>
          <p className="mt-2 font-display text-lg font-black">
            {lastLeg.arrival_airport_code || lastLeg.arrival_airport_name}
          </p>
          <p className="mt-1 text-sm text-muted">
            Fixed to the previous flight's arrival so this journey stays connected.
          </p>
        </div>
        ...
        <label className="form-label">
          ...
          <input
            ...
            defaultValue={isoToLocalDateTime(
              new Date(
                new Date(lastLeg.scheduled_arrival_at).getTime() + 2 * 60 * 60 * 1_000
              ).toISOString(),
              lastLeg.arrival_timezone
            )}
            required
          />
        </label>
        <AirportPicker
          name="arrivalName"
          codeName="arrivalCode"
          timezoneName="arrivalTimezone"
          countryName="arrivalCountry"
          label="To airport"
          defaultTimezone={
            journeyScope === "domestic" ? lastLeg.arrival_timezone : trip.primary_timezone
          }
          countryFilter={domesticCountry}
          showManualTimezone={journeyScope !== "domestic"}
        />
        <label className="form-label">
          Arrival (airport local time)
          <input className="form-input" name="arrivalAt" type="datetime-local" required />
        </label>
        <label className="form-label">
          Boarding lead (minutes, optional)
          <input
            className="form-input"
            ...
            min="0"
            max="360"
            placeholder="Minutes before scheduled departure"
          />
        </label>
