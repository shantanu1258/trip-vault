...
const event = (id: string, all: boolean, bookingId?: string): ItineraryItem => ({
  id,
  trip_id: "trip",
...
const cost = (id: string, itineraryId?: string, bookingId?: string): TripCost => ({
  id,
  trip_id: "trip",
  itinerary_item_id: itineraryId ?? null,
  booking_id: bookingId,
  title: id,
  category: "other",
  amount_minor: 100,
...
const document = (
  id: string,
  mode: "shared" | "selected" | "unassigned",
  ids: string[]
): VaultDocument => ({
  id,
  trip_id: "trip",
...
describe("traveler-focused trip workspace", () => {
  it("keeps shared and selected records while hiding another traveler's records", () => {
    const result = filterTravelerWorkspace({
      travelerId: "asha",
      itinerary: [
        event("shared", true, "shared-booking"),
        event("asha-event", false, "asha-booking"),
        event("ravi-event", false, "ravi-booking")
      ],
      participants: [
        { id: "a", itinerary_item_id: "asha-event", traveler_id: "asha" },
        { id: "r", itinerary_item_id: "ravi-event", traveler_id: "ravi" }
      ],
      bookings: [booking("shared-booking"), booking("asha-booking"), booking("ravi-booking")],
      bookingTravelers: [
        { id: "a", booking_id: "asha-booking", traveler_id: "asha" },
        { id: "r", booking_id: "ravi-booking", traveler_id: "ravi" }
      ],
      costs: [
        cost("shared-cost", "shared", "shared-booking"),
        {
          ...cost("asha-cost", "asha-event"),
          participants: [{ traveler_id: "asha", share_amount_minor: null }]
        },
        {
          ...cost("ravi-on-shared", "shared", "shared-booking"),
          participants: [{ traveler_id: "ravi", share_amount_minor: null }]
        },
        cost("ravi-cost", "ravi-event"),
        cost("unlinked")
      ],
      requirements: [
        requirement("everyone-ready"),
        requirement("asha-ready"),
        requirement("ravi-ready")
      ],
      requirementAssignees: [
        { id: "a", requirement_id: "asha-ready", traveler_id: "asha" },
        { id: "r", requirement_id: "ravi-ready", traveler_id: "ravi" }
      ],
      documents: [
        document("shared-doc", "shared", []),
        document("asha-doc", "selected", ["asha"]),
        document("ravi-doc", "selected", ["ravi"]),
        document("unassigned-doc", "unassigned", [])
      ]
    });
    ...
    expect(result.requirements.map((item) => item.id)).toEqual(["everyone-ready", "asha-ready"]);
