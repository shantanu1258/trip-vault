...
export function primaryFlightDocument(documents: VaultDocument[]) {
  return (
    documents.find((document) => document.purpose === "boarding_pass") ??
    documents.find((document) => document.purpose === "ticket") ??
    null
  );
}

export function effectiveDeparture(flight: FlightLeg) {
  return flight.status === "delayed" && flight.estimated_departure_at
...
export function delayMinutes(flight: FlightLeg) {
  ...
  return Math.max(
    0,
    Math.round(
      (new Date(flight.estimated_departure_at).getTime() -
        new Date(flight.scheduled_departure_at).getTime()) /
        60_000
    )
  );
}
...
export function flightCountdown(flight: FlightLeg, now = new Date()) {
  ...
  const minutes = Math.round(
    (new Date(effectiveDeparture(flight)).getTime() - now.getTime()) / 60_000
  );
...
export function resolveBoardingInstant(
  departureAt: string,
  exactBoardingAt?: string | null,
  boardingLeadMinutes?: number | null
) {
...
export function flightSeatLabels(
  travelers: Traveler[],
  details: FlightTraveler[],
  focusedTravelerId?: string | null
) {
  const visible = focusedTravelerId
    ? travelers.filter((traveler) => traveler.id === focusedTravelerId)
    : travelers;
  const byTraveler = new Map(details.map((row) => [row.traveler_id, row]));
  return visible.map((traveler) => ({
    travelerId: traveler.id,
    travelerName: traveler.display_name,
    seat: byTraveler.get(traveler.id)?.seat ?? null
  }));
}
