  ...
  return (
    <section className="surface-card p-5 sm:p-6">
      ...
      <p className="mt-3 text-sm leading-6 text-muted">
        The file is saved under this login before any trip association is attempted. Unfinished
        files stay here until you attach or delete them.
      </p>
      <form className="mt-5 space-y-3" onSubmit={submitUpload}>
        <FileDropzone
          name="inboxFile"
          label="PDF or image under 5 MB"
          prompt="Choose a private PDF or image"
          file={selectedFile}
          onFileChange={(file) => {
          ...
          description="Save it now, then choose its trip and travelers when you are ready"
        />
        <button className="primary-button w-full" disabled={stage.isPending}>
          {stage.isPending ? (
      ...
      {(message || stage.error || remove.error || retry.error) && (
        <p
          role="status"
          className={`mt-3 rounded-xl p-3 text-sm font-bold ${stage.error || remove.error || retry.error ? "bg-danger/10 text-danger" : "bg-success/10 text-success"}`}
        >
          {stage.error || remove.error || retry.error
            ? getErrorMessage(stage.error || remove.error || retry.error)
            : message}
        </p>
      )}
      ...
      {selected && (
        <AssociateInboxDocumentSheet
          ...
          onAssociated={async () => {
            setSelected(null);
            await Promise.all([
              queryClient.invalidateQueries({ queryKey: ["account-document-uploads"] }),
              queryClient.invalidateQueries({ queryKey: ["documents"] }),
              queryClient.invalidateQueries({ queryKey: ["documents", selectedTripId] })
            ]);
          }}
        />
      )}
...
function AssociateInboxDocumentSheet({
  upload,
  tripId,
  onTripChange,
  trips,
  ...
  const profileId = useQuery({
    ...
    queryFn: async () => (await localProfileId()) ?? null
  });
  ...
  const defaultTitle = useMemo(
    ...
    [assignmentMode, kind, selectedTravelerIds, selectedTrip?.title, travelers.data]
  );
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    setMessage("");
    if (!tripId) {
      setMessage("Choose a trip.");
      return;
    }
    if (assignmentMode === "selected" && !selectedTravelerIds.length) {
    ...
    const validSelectedUserIds =
      effectiveVisibility === "selected_members"
        ? selectedUserIds.filter((id) => memberIds.has(id))
        : [];
    if (effectiveVisibility === "selected_members" && !validSelectedUserIds.length) {
      setMessage("Choose at least one signed-in member.");
      return;
    }
    if (
      effectiveVisibility === "selected_members" &&
      validSelectedUserIds.length !== selectedUserIds.length
    ) {
      setMessage("Every selected member must belong to this trip.");
      return;
    }
  ...
  return (
    <ModalSheet eyebrow="Private document inbox" title="Attach uploaded file" onClose={onClose}>
      <form className="mt-6 space-y-4" onSubmit={submit}>
        <div className="rounded-2xl bg-elevated p-4">
          <p className="truncate font-extrabold">{upload.original_filename}</p>
          <p className="mt-1 text-xs text-muted">
            The private file already exists; this step only adds trip metadata.
          </p>
        </div>
        <label className="form-label">
          ...
          <select
            ...
            value={tripId}
            onChange={(event) => {
              onTripChange(event.target.value);
        ...
        <label className="form-label">
          ...
          <select
            ...
            onChange={(event) => {
              ...
              setKind(next);
              setAssignmentMode(documentKind(next).defaultAssignment);
              setSelectedTravelerIds([]);
            }}
          >
          ...
          <div className="mt-2 grid gap-2 sm:grid-cols-3">
            {(
              [
                ...
                { value: "selected", label: "Traveler(s)" },
                { value: "unassigned", label: "Assign later" }
              ] as const
            ).map((option) => (
              <label
                key={option.value}
                className={`cursor-pointer rounded-xl border p-3 text-sm font-bold ${assignmentMode === option.value ? "border-brand bg-brand-soft" : "border-line"}`}
              >
                <input
                  className="sr-only"
                  type="radio"
        ...
        <label className="form-label">
          ...
          <input
            ...
            placeholder="Leave empty to use type, traveler, and trip"
          />
        </label>
        <div className="rounded-xl bg-brand-soft p-3 text-sm">
          <span className="text-muted">Saved name:</span>{" "}
          <strong>{customTitle.trim() || defaultTitle}</strong>
        </div>
        <label className="form-label">
          ...
          <input
            className="form-input"
            name="shortLabel"
            placeholder="Seat, bag, ticket, or reference detail"
          />
        </label>
        {checkingPermissions ? (
          <p className="flex items-center gap-2 rounded-2xl bg-elevated p-4 text-sm text-muted">
            <Loader2 className="size-4 animate-spin" /> Checking your trip access
          </p>
        ) : canShare ? (
          <>
            <label className="form-label">
              ...
              <select
                ...
                value={visibility}
                onChange={(event) => {
                  const next = event.target.value as DocumentVisibility;
                  setVisibility(next);
                  if (next !== "selected_members") setSelectedUserIds([]);
                }}
              >
          ...
          <>
            ...
            <p className="rounded-2xl bg-brand-soft p-4 text-sm text-muted">
              Only you can open this document. Your trip role does not allow sharing it with other
              signed-in members.
            </p>
          </>
        )}
        <p className="text-xs leading-5 text-muted">
          This inbox file stays private until you attach it. The resulting Vault document uses the
          access choice above and can still be attached to a timeline event.
        </p>
        {(message || associate.error) && (
          <p role="alert" className="rounded-xl bg-danger/10 p-3 text-sm font-bold text-danger">
            {message || getErrorMessage(associate.error)}
          </p>
        )}
        <button
          className="primary-button w-full"
          disabled={associate.isPending || !tripId || checkingPermissions}
        >
          {associate.isPending ? (
            <Loader2 className="size-4 animate-spin" />
          ) : (
