...
const savedLeg: JourneyLeg = {
  ...
  boarding_at: "2026-09-30T00:30:00.000Z",
  boarding_lead_minutes: 30,
  departure_platform: "Exit D",
  arrival_platform: null,
  coach_or_cabin: null,
  seat: null,
  details: {
    kind: "bus",
    bus_class_or_layout: "Executive (2+1)",
    shared_ticket_number: "SGV8G20684227"
  },
  status_note: null,
  version: 4
};
...
const savedBooking: Booking = {
  id: savedLeg.booking_id,
  trip_id: "33333333-3333-4333-8333-333333333333",
  type: "bus",
  title: "Singapore to Kuala Lumpur",
  provider: "Qistna Express",
  reference_code: "I7UCJLFR",
  start_at: savedLeg.scheduled_departure_at,
  end_at: savedLeg.scheduled_arrival_at,
  source_timezone: savedLeg.origin_timezone,
  location: null,
  details: {},
...
beforeEach(() => {
  ...
  mocks.rpc.mockResolvedValue({
    data: {
      ...
      itinerary_items: [{ id: "item-1", trip_id: savedBooking.trip_id, title: savedBooking.title }]
    },
    error: null
  });
});
...
describe("updateJourneyLeg", () => {
  it("uses the atomic RPC and refreshes all three offline caches", async () => {
    ...
    expect(mocks.cacheEntity).toHaveBeenCalledWith(
      `bookings:${savedBooking.trip_id}`,
      savedBooking
    );
    expect(mocks.cacheEntity).toHaveBeenCalledWith(
      `itinerary:${savedBooking.trip_id}`,
