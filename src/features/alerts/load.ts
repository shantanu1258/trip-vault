...
export async function loadAlertInputs(queryClient?: QueryClient) {
  ...
  const [
    ...
    Promise.all(
      trips.map((trip) =>
        queryClient ? queryClient.fetchQuery(tripQueries.bookings(trip.id)) : listBookings(trip.id)
      )
    ),
    Promise.all(
      trips.map((trip) =>
        queryClient
          ? queryClient.fetchQuery(tripQueries.requirements(trip.id))
          : listRequirements(trip.id)
      )
    ),
    Promise.all(
      trips.map((trip) =>
        queryClient
          ? queryClient.fetchQuery(tripQueries.itinerary(trip.id))
    ...
    Promise.all(
      trips.map((trip) =>
        queryClient
          ? queryClient.fetchQuery(tripQueries.documents(trip.id))
          : listVaultDocuments(trip.id)
      )
    ),
  ...
  const flightGroups = await Promise.all(
  ...
  return {
    ...
    conflicts: outbox
      .filter(
        (operation) =>
          operation.lastErrorCode === "conflict" || operation.lastErrorCode === "version_conflict"
      )
      .map((operation) => ({ entityId: operation.entityId, entityType: operation.entityType }))
  };
}
