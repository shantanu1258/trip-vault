...
export async function loadAlertInputs() {
  ...
  const [flightGroups, requirementGroups, itineraryGroups, documentGroups, offlineManifests, outbox] = await Promise.all([
    ...
    Promise.all(trips.map((trip) => listItinerary(trip.id))),
