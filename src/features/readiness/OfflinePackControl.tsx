...
export function OfflinePackControl({ tripId }: { tripId: string }) {
  ...
  useEffect(() => {
    getOfflineManifest(tripId).then(setManifest);
  }, [tripId]);
  const prepare = async (essentialsOnly = false) => {
    setWorking(true);
    setError("");
    try {
      setManifest(
        await prepareTripOffline(
          tripId,
      ...
      setError(reason instanceof Error ? reason.message : "Offline preparation failed.");
    } finally {
      setWorking(false);
    }
  };
  const remove = async () => {
    if (
      !(await confirm({
        ...
        message:
          "Remove this trip's prepared files from this browser? Cloud originals will not be deleted.",
    ...
    <section className={`surface-card p-5 ${ready ? "border-success/30" : ""}`}>
        ...
        <div className="mt-4">
          ...
          <div className="mt-2 h-2 overflow-hidden rounded-full bg-line">
            <div
              ...
              style={{ width: `${progress[1] ? (progress[0] / progress[1]) * 100 : 15}%` }}
            />
          </div>
        </div>
      )}
      {error && (
        <p role="alert" className="mt-3 flex gap-2 text-sm font-bold text-danger">
      ...
      {manifest?.checkedAt && (
        <p className="mt-3 text-xs text-muted">
          Checked {new Date(manifest.checkedAt).toLocaleString()}
        </p>
      )}
    </section>
  );
