...
export function SyncIssuesPanel() {
  ...
  const mutation = useMutation({
    ...
    }) => resolveSyncIssue(issue.operationId, resolution),
    onSuccess: async () => {
      await Promise.all([
    ...
    <section className="surface-card p-5 sm:p-6">
      ...
      <div className="mt-4 space-y-4">
        {query.data.map((issue) => (
          <article key={issue.operationId} className="rounded-2xl border border-warning/40 p-4">
            <p className="text-sm font-black capitalize">{issue.entityType.replaceAll("-", " ")}</p>
            <p className="mt-1 text-xs text-muted">
              {issue.lastErrorCode?.replaceAll("_", " ")} · attempt {issue.attemptCount}
            </p>
            {issue.lastErrorCode === "conflict" && (
              ...
              {issue.lastErrorCode === "conflict" ? (
                <>
                  <button
                    ...
                    onClick={() => mutation.mutate({ issue, resolution: "keep_local" })}
                  >
                  ...
                  <button
                    ...
                    className="secondary-button"
                    onClick={() => mutation.mutate({ issue, resolution: "use_server" })}
                  >
                ...
                <>
                  <button
                    disabled={mutation.isPending || !navigator.onLine}
                    className="secondary-button"
                    onClick={() => mutation.mutate({ issue, resolution: "retry" })}
                  >
                    {mutation.isPending ? (
                    ...
                    onClick={async () => {
                      if (
                        await confirm({
                          ...
                          message: "Discard this saved change and any changes that depend on it?",
                          confirmLabel: "Discard",
                          tone: "danger"
                        })
                      )
                        mutation.mutate({ issue, resolution: "discard" });
                    }}
                  >
