...
import { classifySyncError, orderOutbox } from "./localSync";
...
describe("foreground synchronization", () => {
  ...
  it("classifies conflicts, quota, authentication, permissions, retryable failures, and generic failures", () => { expect(classifySyncError(new Error("version_conflict"))).toBe("conflict"); expect(classifySyncError(new Error("Not enough space"))).toBe("quota"); expect(classifySyncError(new Error("session expired"))).toBe("authentication"); expect(classifySyncError(new Error("row level security policy"))).toBe("permission"); expect(classifySyncError(new Error("network timeout"))).toBe("retryable"); expect(classifySyncError(new Error("bad value"))).toBe("failed"); });
});
