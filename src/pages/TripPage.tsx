...
import { costsForEvent, eventEndDetails, eventTimeLabel, journeyDuration, mapsUrl, phoneActionUrls, readinessSummary, resolveCurrentTimelineItem, searchTrip, sortTimelineItems, timelinePhase, type TimelinePhase } from "../features/timeline/model";
...
import type { EventStatus, ItineraryItem, TimelineEventType, Trip, TripCost } from "../features/trips/types";
...
function EventDetailsSheet({ item, tripId, booking, flights, journeys, travelerIds, travelers, costs, focusedTravelerId, editable, canMoveUp, canMoveDown, onClose, onEdit, onArchive, onAddCost, onEditCost, onUploadDocument, onStatus, onMoveUp, onMoveDown }: { item: ItineraryItem; tripId: string; booking?: Booking; flights: FlightLeg[]; journeys: JourneyLeg[]; travelerIds: string[]; travelers: Traveler[]; costs: TripCost[]; focusedTravelerId?: string | null; editable: boolean; canMoveUp: boolean; canMoveDown: boolean; onClose: () => void; onEdit: () => void; onArchive: () => void; onAddCost: () => void; onEditCost: (cost: TripCost) => void; onUploadDocument: () => void; onStatus: (status: EventStatus) => void; onMoveUp: () => void; onMoveDown: () => void }) {
  const Icon = iconFor[item.event_type ?? "custom"];
  const map = mapsUrl(item.location); const end = eventEndDetails(item); const timingLabel = eventTimeLabel(item);
  return <ModalSheet eyebrow={(item.event_type ?? "event").replaceAll("_", " ")} title={item.title} onClose={onClose}>
    <div className="mt-5 flex items-start gap-4 rounded-2xl bg-elevated p-4"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-brand-soft text-brand"><Icon className="size-5" /></span><div className="min-w-0"><p className="text-sm font-black">{timingLabel ?? formatEventTime(item.starts_at, item.timezone)}</p><p className="mt-1 text-xs text-muted">{item.timing_mode === "unscheduled" ? "Schedule this when your plan is clearer" : item.timezone}</p>{end && <p className="mt-2 text-xs text-muted">{end.journey ? "Arrives" : "Ends"} {formatEventTime(end.endsAt, item.timezone)} · {end.duration}</p>}<span className="mt-2 inline-flex rounded-full bg-surface px-2 py-1 text-[.65rem] font-black uppercase text-muted">{(item.event_status ?? "planned").replaceAll("_", " ")}</span></div></div>
...
export function TripPage() {
  ...
  const openExpenses = () => { if (view !== "details") changeView("details"); window.setTimeout(() => document.getElementById("costs")?.scrollIntoView({ behavior: preferredScrollBehavior(), block: "start" }), 50); };
  ...
  return <AppShell><div className="mx-auto max-w-6xl pb-24">
    ...
    {trip && <>
      <header className="page-enter mt-4 rounded-[2rem] bg-brand p-5 text-surface shadow-focus sm:p-7"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-surface/60">{formatDateRange(trip.start_date, trip.end_date)}</p><h1 className="mt-2 font-display text-3xl font-black tracking-[-.05em] sm:text-4xl">{trip.title}</h1><p className="mt-2 flex items-center gap-2 text-sm text-surface/70"><MapPin className="size-4" />{trip.destination_summary}</p></div><span className="rounded-full bg-surface/10 px-3 py-2 text-xs font-bold capitalize">{role ?? "member"}</span></div>
        ...
        <button type="button" onClick={openExpenses} className="mt-3 block w-full rounded-2xl bg-surface/10 p-1 text-left transition hover:bg-surface/15"><CostTotals costs={visibleCosts} emptyText="Add your first trip cost" /></button>
      </header>
              ...
              return <Fragment key={item.id}>
                ...
                <div id={`timeline-${item.id}`} className="relative mb-4 grid scroll-mt-28 grid-cols-1 pl-5 sm:grid-cols-[6rem_2.5rem_minmax(0,1fr)] sm:gap-4 sm:pl-0">
                  ...
                  <FocusSurface active={current} role="button" tabIndex={0} onClick={() => setViewingItineraryId(item.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setViewingItineraryId(item.id); } }} className={`group cursor-pointer p-4 pr-16 outline-none hover:-translate-y-0.5 hover:border-brand/40 hover:shadow-soft focus-visible:ring-2 focus-visible:ring-brand sm:p-5 ${current ? "bg-coral/10" : "bg-elevated"}`} aria-label={`Open details for ${item.title}`}>
                    ...
                    <div className="min-w-0"><span className={`mb-2 inline-flex rounded-full px-2 py-1 text-[.6rem] font-black uppercase tracking-[.12em] ${current ? "bg-coral text-white" : "bg-surface text-muted"}`}>{phaseForActive(item, current)}</span><h3 className="font-display text-lg font-black">{item.title}</h3><p className="mt-1 text-xs capitalize text-muted">{(item.event_type ?? "custom").replaceAll("_", " ")} · {(item.event_status ?? "planned").replaceAll("_", " ")}</p>{end && <p className="mt-1 text-xs text-muted">{end.journey ? "Arrives" : "Ends"} {formatEventTime(end.endsAt, item.timezone)} · {end.duration}</p>}</div>
                    <EventTravelers item={item} travelerIds={travelerIds} travelers={travelers} />
                    ...
                    <div className="mt-3 flex flex-wrap gap-3">{map ? <a href={map} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()} className="inline-flex items-center gap-1.5 text-xs font-extrabold text-brand"><LocateFixed className="size-4" /> Navigation</a> : editable ? <button type="button" onClick={(event) => { event.stopPropagation(); setEditingItinerary(item); }} className="inline-flex items-center gap-1.5 text-xs font-extrabold text-warning"><MapPin className="size-4" /> Add location</button> : null}</div>
