...
import { calculateTripBalances, splitExpenseEqually, type TravelerBalance } from "../features/trips/expenses";
...
export function CostDetailsSheet({ cost, travelers, itinerary, bookings = [], editable, onClose, onEdit, onArchive }: { cost: TripCost; travelers: Traveler[]; itinerary: ItineraryItem[]; bookings?: Booking[]; editable: boolean; onClose: () => void; onEdit: () => void; onArchive: () => void }) {
  ...
  return <ModalSheet eyebrow="Trip expense" title={cost.title} onClose={onClose}>
    ...
    <div className="mt-3 rounded-xl bg-elevated p-3"><p className="text-xs font-bold text-muted">Included travelers</p>{participants.length ? <ul className="mt-2 space-y-1.5">{participants.map((participant, index) => <li className="flex items-center justify-between gap-3 text-sm" key={`${participant.name}:${index}`}><strong>{participant.name}</strong><span className="text-muted">{participant.share === null ? "Equal share" : formatMoney(participant.share, cost.currency_code)}</span></li>)}</ul> : <p className="mt-1 text-sm font-extrabold">No travelers selected</p>}</div>
    ...
    {editable && <div className="mt-6 grid grid-cols-2 gap-3 border-t border-line pt-5"><button type="button" className="secondary-button" onClick={onEdit}><Pencil className="size-4" /> Edit expense</button><button type="button" className="secondary-button text-danger" onClick={onArchive}><Trash2 className="size-4" /> Archive</button></div>}
  </ModalSheet>;
}
...
export function TripExpensesContent({ costs, balances, travelers, onViewCost }: { costs: TripCost[]; balances: TravelerBalance[]; travelers: Traveler[]; onViewCost: (cost: TripCost) => void }) {
  ...
  return <>
    <div className="flex flex-wrap items-start gap-3"><div className="min-w-0 flex-1"><CostTotals costs={costs} /></div><label className="flex min-h-11 items-center gap-2 rounded-xl border border-line px-3 text-xs font-extrabold text-muted"><input type="checkbox" checked={showBalances} onChange={(event) => setShowBalances(event.target.checked)} /> Show balances</label></div>
    ...
    <div className="mt-4 space-y-2">{costs.map((cost) => <button type="button" onClick={() => onViewCost(cost)} className="group grid w-full cursor-pointer grid-cols-[minmax(0,1fr)_auto] items-center gap-x-3 gap-y-1 rounded-xl bg-elevated p-3 text-left text-sm transition hover:-translate-y-0.5 hover:shadow-soft focus-visible:ring-2 focus-visible:ring-brand" key={cost.id} aria-label={`View details for ${cost.title}`}><span className="min-w-0"><span className="block truncate font-bold">{cost.title}</span>{cost.paid_by_traveler_id && <span className="mt-0.5 block truncate text-xs text-muted">Paid by {travelers.find((traveler) => traveler.id === cost.paid_by_traveler_id)?.display_name ?? "traveler"}</span>}</span><span className="flex items-center gap-2"><strong className="whitespace-nowrap">{cost.amount_minor === 0 ? "Free" : formatMoney(cost.amount_minor, cost.currency_code)}</strong><ChevronRight className="size-4 shrink-0 text-muted transition-transform group-hover:translate-x-0.5" /></span></button>)}</div>
  </>;
}
...
export function TripPage() {
  ...
  const chooseTraveler = (id: string | null) => { setFocusedTravelerId(id); writeTravelerFocus(tripId, id); positioned.current = false; setViewingItineraryId(null); setViewingCost(null); setCostReturnEventId(null); };
  ...
  const closeCostDetails = () => { const returnEventId = costReturnEventId; setViewingCost(null); setCostReturnEventId(null); if (returnEventId) setViewingItineraryId(returnEventId); };
