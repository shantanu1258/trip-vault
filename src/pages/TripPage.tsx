...
import { archiveItineraryItem, archiveTripCost, getTrip, listArchivedTripItems, listCosts, listItinerary, reorderItineraryItems, restoreItineraryItem, restoreTripCost, setItineraryItemStatus, updateTripExpenseSplitting } from "../features/trips/api";
...
export function TripPage() {
    ...
    {trip && <>
      ...
      {view === "timeline" ? <main className="mt-5">
        ...
        <Section id="costs" eyebrow="Money" title={focusedTraveler ? `${focusedTraveler.display_name}'s trip costs` : "Trip expenses"} onActivate={() => openExpenses()} activateLabel="Open itemized trip expenses" action={editable && <button type="button" className="secondary-button" onClick={() => { setCostTargetItem(null); setOpenForm("cost"); }}><Plus className="size-4" /> Add</button>}>
          {editable && <label className="mb-4 flex cursor-pointer items-start gap-3 rounded-xl border border-line bg-elevated p-3 text-sm" onClick={(event) => event.stopPropagation()}><input type="checkbox" className="mt-0.5 size-5 shrink-0 accent-brand" checked={Boolean(trip.expense_splitting_enabled)} disabled={updateExpenseSplitting.isPending} onChange={(event) => updateExpenseSplitting.mutate({ trip, enabled: event.target.checked })} /><span><strong className="block">Enable expense splitting</strong><span className="mt-1 block text-xs leading-5 text-muted">Choose which travelers share each cost. When off, costs apply equally to everyone.</span></span></label>}
          {updateExpenseSplitting.error && <p role="alert" className="mb-4 rounded-xl bg-danger/10 p-3 text-sm font-bold text-danger">{getErrorMessage(updateExpenseSplitting.error)}</p>}
  ...
  {trip && openForm === "event" && editable && <AddEventForm trip={trip} travelers={travelers} preferredTravelerId={focusedTravelerId ?? undefined} onClose={closeForm} onAddDocument={(saved, handoff) => { setDocumentTargetItem({ id: saved.itineraryItemId, booking_id: saved.bookingId, title: saved.title, initialFile: handoff?.file, initialKind: handoff?.kind }); setOpenForm("document"); }} />}
  ...
  {trip && openForm === "document" && <UploadDocumentForm trip={trip} travelers={travelers} preferredTravelerId={focusedTravelerId ?? undefined} members={members} privateOnly={!editable} bookingId={documentTargetItem?.booking_id ?? undefined} contextTitle={documentTargetItem?.title} initialFile={documentTargetItem?.initialFile} initialKind={documentTargetItem?.initialKind} onUploaded={documentTargetItem ? async (documentId) => { const target = itinerary.find((item) => item.id === documentTargetItem.id) ?? (await listItinerary(trip.id)).find((item) => item.id === documentTargetItem.id); if (!target) throw new Error("The document was saved, but its new timeline event could not be linked. Open the event and attach it from there."); await attachDocumentsToEvent(target, [documentId]); await queryClient.invalidateQueries({ queryKey: ["event-documents", documentTargetItem.id] }); } : undefined} onClose={closeForm} />}
