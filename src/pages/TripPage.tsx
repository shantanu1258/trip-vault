...
export function TripPage() {
  ...
  const visibleItinerary = focusedWorkspace.itinerary; const visibleBookings = focusedWorkspace.bookings; const visibleCosts = focusedWorkspace.costs; const visibleRequirements = focusedWorkspace.requirements; const focusedDocuments = focusedWorkspace.documents;
  ...
  return <AppShell><div className="mx-auto max-w-6xl pb-24">
    ...
    {trip && <>
      ...
      {view === "timeline" ? <main className="mt-5">
        ...
        <Section id="costs" eyebrow="Money" title={focusedTraveler ? `${focusedTraveler.display_name}'s trip costs` : "Trip costs"} action={editable && <button type="button" className="secondary-button" onClick={() => { setCostTargetItem(null); setOpenForm("cost"); }}><Plus className="size-4" /> Add</button>}><CostTotals costs={visibleCosts} /><div className="mt-4 space-y-2">{visibleCosts.map((cost) => <div className="flex items-center gap-2 rounded-xl bg-elevated p-3 text-sm" key={cost.id}><span className="min-w-0 flex-1 truncate">{cost.title}</span><strong>{cost.amount_minor === 0 ? "Free" : formatMoney(cost.amount_minor, cost.currency_code)}</strong>{editable && <><button type="button" onClick={() => setEditingCost(cost)} aria-label={`Edit ${cost.title}`}><Pencil className="size-4" /></button><button type="button" className="text-danger" onClick={() => window.confirm(`Archive ${cost.title}?`) && archiveCost.mutate(cost)} aria-label={`Archive ${cost.title}`}><Trash2 className="size-4" /></button></>}</div>)}</div></Section>
  ...
  {trip && openForm === "cost" && editable && <AddCostForm trip={trip} bookingId={costTargetItem?.booking_id ?? undefined} itineraryItemId={costTargetItem?.id} sourceTitle={costTargetItem?.title} onClose={closeForm} />}
