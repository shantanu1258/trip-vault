...
import { formatDateRange, formatEventTime, selectFocusedTrip, sortTripsByRelevance, tripPhase } from "../features/trips/presentation";
...
export function HomePage() {
  ...
  const costsQuery = useQuery({ ...tripQueries.costs(focusedTripId), enabled: Boolean(focusedTrip) });
  const itineraryQuery = useQuery({ ...tripQueries.itinerary(focusedTripId), enabled: Boolean(focusedTrip) });
  const bookingsQuery = useQuery({ ...tripQueries.bookings(focusedTripId), enabled: Boolean(focusedTrip) });
  const flightsQuery = useQuery({ ...tripQueries.flights(focusedTripId, bookingsQuery.data), enabled: Boolean(focusedTrip) && bookingsQuery.isSuccess });
  const requirementsQuery = useQuery({ ...tripQueries.requirements(focusedTripId), enabled: Boolean(focusedTrip) });
  const travelersQuery = useQuery({ ...tripQueries.travelers(focusedTripId), enabled: Boolean(focusedTrip) });
  const documentsQuery = useQuery({ ...tripQueries.documents(focusedTripId), enabled: Boolean(focusedTrip) });
        ...
        {focusedTrip && (
          <>
            <section className={`page-enter group relative mt-7 overflow-hidden rounded-[2rem] border p-6 shadow-focus transition-transform duration-200 hover:-translate-y-0.5 sm:p-8 motion-reduce:hover:translate-y-0 ${tripPhase(focusedTrip) === "current" ? "border-coral bg-brand text-surface motion-safe:scale-[1.01]" : "border-line bg-surface"}`}>
              ...
              <Link to={`/trips/${focusedTrip.id}`} className={`tap-target relative z-20 mt-6 inline-flex items-center gap-2 rounded-2xl px-5 py-3 text-sm font-extrabold ${tripPhase(focusedTrip) === "current" ? "bg-surface text-brand" : "bg-brand text-surface"}`}>Open trip <ArrowRight className="size-4 transition-transform group-hover:translate-x-0.5 motion-reduce:transition-none" /></Link>
            </section>
            ...
            <section className="mt-5 grid gap-4 md:grid-cols-2">
              <section className="surface-card page-enter group relative p-5 transition hover:-translate-y-0.5 hover:border-brand/40 hover:shadow-soft motion-reduce:hover:translate-y-0"><button type="button" onClick={() => setShowingExpenses(true)} className="absolute inset-0 z-10 rounded-[inherit] text-left outline-none focus-visible:ring-2 focus-visible:ring-brand" aria-label="Open trip expenses" /><p className="eyebrow">Trip expenses</p><div className="mt-3 flex items-center justify-between gap-3"><CompactCostTotal costs={costsQuery.data ?? []} emptyText="No costs added yet" /><ArrowRight className="size-4 shrink-0 text-brand transition-transform group-hover:translate-x-0.5 motion-reduce:transition-none" /></div><p className="mt-2 text-xs text-muted">Open the itemized costs, payers, participants, and balances.</p><Link to={`/trips/${focusedTrip.id}?add=cost`} className="relative z-20 mt-3 inline-flex items-center gap-2 text-sm font-extrabold text-brand"><Plus className="size-4" /> Add cost</Link></section>
              <section className="surface-card page-enter group relative p-5 transition hover:-translate-y-0.5 hover:border-brand/40 hover:shadow-soft motion-reduce:hover:translate-y-0"><Link to={`/trips/${focusedTrip.id}/readiness`} className="absolute inset-0 z-10 rounded-[inherit] focus-visible:ring-2 focus-visible:ring-brand" aria-label="Open trip readiness" /><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.12em] text-muted"><Sparkles className="size-4" /> Readiness checklist</div><p className="mt-3 font-display text-xl font-black">{(requirementsQuery.data?.filter((item) => !["complete", "not_required"].includes(item.status)).length ?? 0) > 0 ? `${requirementsQuery.data?.filter((item) => !["complete", "not_required"].includes(item.status)).length} task${requirementsQuery.data?.filter((item) => !["complete", "not_required"].includes(item.status)).length === 1 ? "" : "s"} left` : (requirementsQuery.data?.length ?? 0) > 0 ? "Everything is done" : "Add your first task"}</p><p className="mt-2 text-sm leading-6 text-muted">{requirementsQuery.data?.find((item) => !["complete", "not_required"].includes(item.status)) ? `Next: ${requirementsQuery.data.find((item) => !["complete", "not_required"].includes(item.status))?.title}` : (requirementsQuery.data?.length ?? 0) > 0 ? "Every item on your checklist is complete." : "Keep a simple list of what needs to be done before you leave."}</p><Link to={`/trips/${focusedTrip.id}/readiness`} className="relative z-20 mt-3 inline-flex items-center gap-2 text-sm font-extrabold text-brand">Open checklist <ArrowRight className="size-4 transition-transform group-hover:translate-x-0.5 motion-reduce:transition-none" /></Link></section>
            </section>
