...
export function JoinPage() {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (normalized.length === 16) mutation.mutate(normalized);
  };
  return (
    <AppShell>
      <div className="mx-auto max-w-xl">
        ...
        <section className="surface-card page-enter mt-5 overflow-hidden">
          <div className="bg-brand p-7 text-surface">
            ...
            <h1 className="mt-2 font-display text-3xl font-black">Join a trip</h1>
            <p className="mt-2 text-sm leading-6 text-surface/70">
              Enter the unique code the organizer created for you. Nothing about the trip is
              disclosed until the code succeeds.
            </p>
          </div>
          <form onSubmit={submit} className="p-7">
            <label className="form-label">
              ...
              <input
                autoFocus
                className="form-input mt-2 text-center font-mono text-xl font-black uppercase tracking-[.16em]"
                value={code}
            ...
            <p className="mt-3 flex items-start gap-2 text-xs leading-5 text-muted">
              <ShieldCheck className="mt-0.5 size-4 shrink-0 text-success" /> You must be online. A
              code can be used once and normally expires after 14 days.
            </p>
            {mutation.error && (
              <p
                role="alert"
            ...
            <button
              ...
              disabled={mutation.isPending || normalizeJoinCode(code).length !== 16}
            >
              {mutation.isPending ? (
