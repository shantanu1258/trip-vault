...
vi.mock("../components/DocumentPreview", () => ({
  DocumentPreview: ({ filename, mimeType }: { filename: string; mimeType: string }) => (
...
describe("DocumentPage", () => {
  ...
  it("loads the local file into the in-app preview and keeps clear Open and Info actions", async () => {
    ...
    render(
      <MemoryRouter
        initialEntries={[
          {
            pathname: "/trips/trip-1/documents/document-1",
  ...
  it("lets an owner change an existing document to selected-member access", async () => {
    ...
    mocks.listMembers.mockResolvedValue([
      { user_id: "user-1", role: "owner", display_name: "Owner" },
      { user_id: "user-2", role: "viewer", display_name: "Friend" }
    ]);
    const client = new QueryClient({
      defaultOptions: { queries: { retry: false }, mutations: { retry: false } }
    });
    render(
      <MemoryRouter initialEntries={["/trips/trip-1/documents/document-1"]}>
        <QueryClientProvider client={client}>
          <Routes>
            <Route path="/trips/:tripId/documents/:documentId" element={<DocumentPage />} />
          </Routes>
    ...
    expect(mocks.updateDocumentVisibility).toHaveBeenCalledWith({
