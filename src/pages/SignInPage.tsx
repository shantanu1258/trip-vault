...
export function SignInPage({ admin = false }: { admin?: boolean }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!supabase) {
      setError(
        "Supabase is not configured yet. Add the two browser-safe values from .env.example, or explore the demo trip now."
      );
      return;
    }
      ...
      <main className="mx-auto grid max-w-5xl items-center gap-10 px-5 pb-16 pt-6 sm:px-8 md:min-h-[75dvh] md:grid-cols-2">
        <section>
          ...
          {!admin && (
            ...
            >
              Explore the safe demo instead <span aria-hidden="true">→</span>
            </Link>
          )}
        </section>
        ...
        <section className="surface-card p-5 sm:p-8">
          {!isSupabaseConfigured && (
            <div className="mb-5 rounded-2xl border border-warning/30 bg-warning/10 p-4 text-sm">
              ...
              <p className="mt-1 leading-6 text-muted">
                Live sign-in activates after the Supabase URL and publishable key are added locally.
              </p>
            </div>
          )}
          <form onSubmit={submit} className="space-y-5">
            {mode === "sign-up" && (
              <div>
                ...
                <div className="relative mt-2">
                  <UserRound className="pointer-events-none absolute left-4 top-1/2 size-5 -translate-y-1/2 text-muted" />
                  <input
                    id="display-name"
                    type="text"
                    autoComplete="name"
                    ...
                    placeholder="Name shown to your travel group"
            ...
            <div>
              ...
              <div className="relative mt-2">
                ...
                <input
                  ...
                  className="h-[3.25rem] w-full rounded-2xl border border-line bg-elevated py-3 pl-12 pr-14 text-ink placeholder:text-muted/60"
                />
                <button
                  type="button"
                  ...
                  className="tap-target absolute right-1 top-1/2 grid -translate-y-1/2 place-items-center rounded-xl text-muted hover:text-ink"
                >
                  {showPassword ? (
            ...
            {mode === "sign-up" && (
              <div>
                <label htmlFor="confirm-password" className="text-sm font-extrabold">
                  Confirm password
                  <RequiredMark />
                </label>
                <div className="relative mt-2">
            ...
            {error && (
              <div
                role="alert"
                className="rounded-2xl border border-danger/30 bg-danger/10 p-4 text-sm font-semibold text-danger"
              >
                {error}
              </div>
            )}
            {message && (
              <div
                role="status"
                className="rounded-2xl border border-success/30 bg-success/10 p-4 text-sm font-semibold text-success"
              >
                {message}
              </div>
            )}
            <button
              disabled={busy}
              type="submit"
              className="tap-target inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-brand px-5 py-3.5 font-extrabold text-surface disabled:cursor-not-allowed disabled:opacity-60"
            >
              {busy ? (
          ...
          {!admin && (
            <button
              ...
              className="tap-target mt-4 w-full rounded-xl text-sm font-bold text-muted hover:text-ink"
            >
              {mode === "sign-in"
